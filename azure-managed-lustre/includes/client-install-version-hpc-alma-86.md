...
```bash
sudo dnf install --disableexcludes=main --refresh amlfs-lustre-client-2.15.8_39_g2d32b59-$(uname -r | sed -e "s/\.$(uname -p)$//" | sed -re 's/[-_]/\./g')-1
```
...
sudo dnf upgrade kernel-$NEWKERNELVERSION amlfs-lustre-client-2.15.8_39_g2d32b59-$(echo $NEWKERNELVERSION | sed -e "s/\.$(uname -p)$//" | sed -re 's/[-_]/\./g')-1
```
