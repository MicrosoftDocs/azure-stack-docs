...
> [!NOTE]
> DKMS requires kernel headers (`kernel-headers` and `kernel-devel`), GCC, and Make to be installed. The package dependencies handle this requirement automatically, but the initial install takes a few minutes while the kernel module compiles.
