...
```bash
sudo dnf install -y lustre-client-dkms-2.17.0_25_g48fb0ca
```
...
> [!NOTE]
> DKMS requires kernel headers (`kernel-devel`), GCC, and Make to be installed. The package dependencies handle this requirement automatically, but the initial install takes a few minutes while the kernel module compiles.
