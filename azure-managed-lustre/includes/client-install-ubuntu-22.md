...
1. Install the metapackage that matches your running kernel:
   ...
   ```bash
   sudo apt install [!INCLUDE [client-kernel-version-ubuntu](client-kernel-version-ubuntu.md)]=$(uname -r)
   ```
   ...
   ```bash
   apt upgrade linux-image-[new kernel version] [!INCLUDE [client-kernel-version-ubuntu](client-kernel-version-ubuntu.md)]
   ```
