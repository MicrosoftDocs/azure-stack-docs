...
This article describes how to use local identity with Azure Key Vault for Azure Local deployment. For an overview of this deployment model, see [Overview of Azure Local deployment using local identity with Azure Key Vault](./deployment-local-identity-with-key-vault-overview.md).
...
If the extension wasn't installed during deployment, you can manually install it on Azure Arc-enabled servers by following these steps:
...
1. Go to your Azure Local cluster and note the Azure Arc machine names.
 
1. Run the following PowerShell script to install the extension on Azure Arc machines:
