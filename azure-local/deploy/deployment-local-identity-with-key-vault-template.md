...
This article details how to use an Azure Resource Manager (ARM) template in the Azure portal to deploy Azure Local using local identity with Azure Key Vault. The article also contains the prerequisites and the preparation steps required to begin the deployment.

> [!IMPORTANT]
> ARM template deployment of Azure Local systems is targeted for deployments-at-scale. The intended audience for this deployment is IT administrators who have experience deploying Azure Local instances. We recommend that you deploy a [system via the Azure portal](./deployment-local-identity-with-key-vault.md) first, and then perform subsequent deployments via the ARM template.
