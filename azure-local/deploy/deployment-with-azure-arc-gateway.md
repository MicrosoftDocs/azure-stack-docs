...
1. Run the Arc registration script. The script takes a few minutes to run.
    ...
    Invoke-AzStackHciArcInitialization -SubscriptionID $Subscription -ResourceGroup $RG -TenantID $Tenant -Region australiaeast -Cloud "AzureCloud" -Proxy $ProxyServer -ArcGatewayID $ArcgwId -ProxyBypass $ProxyBypassList 
    ```
...
1. Run the initialization script as follows.
    ...
    Invoke-AzStackHciArcInitialization -SubscriptionID $Subscription -ResourceGroup -TenantID $Tenant $RG -Region australiaeast -Cloud "AzureCloud" -ArcGatewayID $ArcgwId
    ```
