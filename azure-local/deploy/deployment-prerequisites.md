...
|DNS server|A DNS server that is used in your environment. Remember that after deployment, DNS servers used by the nodes and ARB cannot be changed. The DNS server used must resolve the Active Directory Domain. The DNS server IP address cannot use an IP from any of the reserved Kubernetes subnets 10.96.0.0/12 or 10.244.0.0/16 <br> For more information, see the **Specify network settings** page in [Deploy via Azure portal](./deploy-via-portal.md#specify-network-settings).|
