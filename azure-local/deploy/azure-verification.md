...
*Azure verification for VMs* makes it possible for supported Azure-exclusive workloads to work outside of the cloud. This feature, modeled after the [IMDS attestation](/azure/virtual-machines/windows/instance-metadata-service?tabs=windows#attested-data) service in Azure, is a built-in platform attestation service that's enabled by default on Azure Local. It helps provide guarantees for these VMs to operate in other Azure environments.

## Benefits available on Azure Local

Azure verification for VMs enables you to use these benefits available only on Azure Local:
...
| Azure Update Manager | Get [Azure Update Manager](/azure/update-manager/overview?branch=main&tabs=azure-arc-vms) at no cost. This service provides a SaaS solution to manage and govern software updates to VMs on Azure Local. | Available automatically for Azure Local VMs created through the Azure Arc resource bridge on Azure Local. With Software Assurance, you can attest your machine by using Windows Server Azure benefits and licenses, and get AUM for free. For more information, see [Azure Update Manager frequently asked questions](/azure/update-manager/update-manager-faq#what-is-the-pricing-for-azure-update-manager). |
| Azure Machine Configuration | [Azure Machine configuration](/azure/governance/machine-configuration) (formerly Azure Policy Guest Configuration) is provided by the Azure Instance Metadata Service (IMDS). It's available at no cost and enables auditing and configuring OS settings as code for machines and VMs. | Requires you to enable [Legacy OS support](#legacy-os-support). Needs Arc agent version 1.39 or later. See [Latest agent release](/azure/azure-arc/servers/agent-release-notes). |
...
You can install Azure CLI on Windows, macOS, and Linux environments. You can also run it in [Azure Cloud Shell](https://shell.azure.com/). This section describes how to use Bash in Azure Cloud Shell. For more information, see [Quickstart for Azure Cloud Shell](/azure/cloud-shell/quickstart).
...
1. In Windows Admin Center, select **Cluster Manager** from the top drop-down menu. Go to the system that you want to activate. Under **Settings**, select **Azure verification for VMs**.

1. To check Azure VM verification machine status:
...
- Under the **Server** tab, if one or more machines appear as **Expired**:
  - If the machine doesn't sync with Azure for more than 30 days, its status appears as **Expired** or **Inactive**. Select **Sync with Azure** to schedule a manual sync.

### Manage benefits activated on VMs

1. To check which benefits are activated on VMs, go to the **VMs** tab.

1. The dashboard shows the number of VMs with:

   - **Active benefits**: These VMs have Azure-exclusive features activated through Azure VM verification.
   - **Inactive benefits**: These VMs have Azure-exclusive features that need further action before activation.
   - **Unknown**: The portal can't determine the eligible benefits for these VMs because Hyper-V data exchange is turned off. See the following troubleshooting section.
   - **No applicable benefits**: These VMs don't have Azure-exclusive features and don't require Azure VM verification.

1. The table displays the **Eligible benefit** that applies to each VM. See the [full list of benefits available on Azure Local](#benefits-available-on-azure-local).
...
- Under the **VMs** tab, if one or more VMs appear as **Inactive benefits**:
  - If the suggested action is to **Install updates**, you might not have the minimum OS version required for the benefit. Update the VM to meet the [version requirements for workloads](#benefits-available-on-azure-local).
  - If the suggested action is to **Turn on Guest Service Interface**, select it and open the context pane to enable the [Hyper-V Guest Service Interface](/virtualization/hyper-v-on-windows/reference/integration-services#hyper-v-powershell-direct-service). This feature is required for VMs to communicate to the host via VMbus.
  - If the suggested action is regarding **legacy OS support**, see [troubleshooting for legacy OS support](#legacy-os-support).

- Under the **VMs** tab, if one or more VMs appear as **Unknown**:
  - If you want to determine the benefits available for these VMs, you can either check the [full list of benefits available on Azure Local](#benefits-available-on-azure-local), or Windows Admin Center can display this information. To access the information through Windows Admin Center, enable [Hyper-V data exchange (KVP)](/virtualization/hyper-v-on-windows/reference/integration-services#hyper-v-data-exchange-service-kvp) for your VMs by selecting the action labeled **Turn on Hyper-V data exchange**.

### [PowerShell](#tab/azure-ps)

### Check machine status of Azure VM verification

- After Azure VM verification is set up, you can view the host status. Check the system property **IMDS Attestation** by running the following command:
...
- If Azure VM verification isn't yet synced and renewed with Azure for one or more machines, it might appear as **Expired** or **Inactive**. Schedule a manual sync:
...
- To check access to Azure VM verification for VMs, run the following command:
   ...
   > [!NOTE]
   > A VM that supports **v2** can communicate with the machine by using VMBus. Conversely, a **v1** supported VM is configured with legacy OS support and can access Azure VM verification via REST. If a VM supports both **v1** and **v2**, the **v2** method (VMBus) is primarily used, but it can fall back to **v1** if **v2** encounters an issue.

### Troubleshoot VMs

- To set up access to Azure VM verification for VMs, enable the [Hyper-V Guest Service Interface](/virtualization/hyper-v-on-windows/reference/integration-services#hyper-v-powershell-direct-service).
...
You can install Azure CLI on Windows, macOS, and Linux environments. You can also run it in [Azure Cloud Shell](https://shell.azure.com/). This section describes how to use Bash in Azure Cloud Shell. For more information, see the [Quickstart for Azure Cloud Shell](/azure/cloud-shell/quickstart).
...
1. To view legacy OS support status on a cluster, run the following command:
...
1. In Windows Admin Center, select **Cluster Manager** from the top drop-down menu. Go to the system that you want to activate. Under **Settings**, select **Azure verifications for VMs**.

1. In the section for **Legacy OS support**, select **Change status**. Select **On** in the context pane. This setting also enables networking access for all existing VMs. You must manually turn on legacy OS support for any new VMs that you create later.
  
1. Select **Change status** to confirm. It might take a few minutes for machines to reflect the changes.

1. When legacy OS support is successfully turned on:
...
You must enable legacy OS networking for any new VMs that you create after the first setup. To manage access for VMs, go to the **VMs** tab. Any VM that requires legacy OS support access appears as **Inactive**. Select the action to **Set up legacy OS networking** for the selected VM, or for all existing VMs on the system.
...
> [!NOTE]
> To successfully enable legacy OS support on Generation 1 VMs, you must first power off the VM to enable the NIC to be added.
...
- Or, if you want to add all existing VMs on setup, run the following command:
...
No. Azure VM verification is a feature built into Azure Local, and you can only use it on Azure Local.

### I just set up Azure VM verification on my system. How do I ensure that Azure VM verification stays active?

- In most cases, you don't need to take any action. Azure Local automatically renews Azure VM verification when it syncs with Azure.
- However, if the system disconnects for more than 30 days and Azure VM verification shows as **Expired**, you can manually sync by using PowerShell and Windows Admin Center. For more information, see [syncing Azure Local](../faq.yml).
...
- However, for legacy VMs that use legacy OS support, you can manually add new VMs to access Azure VM verification by using Windows Admin Center or PowerShell. For more information, see the [preceding instructions](#legacy-os-support).
- You can still delete and migrate VMs as usual. The NIC **AZSHCI_GUEST-IMDS_DO_NOT_MODIFY** still exists on the VM after migration. To clean up the NIC before migration, you can remove VMs from Azure VM verification by using Windows Admin Center or PowerShell by using the preceding instructions for legacy OS support. Alternatively, you can migrate first and manually delete NICs afterwards.
