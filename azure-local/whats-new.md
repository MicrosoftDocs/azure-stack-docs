...
- **Validation improvements**: Validation time during deployment and update is reduced by up to 50%. Validation now resumes from the point of failure (within a three‑hour window) instead of starting over. If validation has already run and completed within a three-hour window, the "Validate environment" step during deployment will have an 'Unknown' status.
