...
- **Overprovisioning alert**: A warning appears if an Azure Local instance is overprovisioned, specifically when there is insufficient compute capacity (memory) to live migrate workloads during an update. You must acknowledge this alert before proceeding with an update, as VM workloads will be paused during the update due to the lack of available compute or memory capacity.
