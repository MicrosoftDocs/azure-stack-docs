---
title: Release notes with fixed and known issues in Azure Local 2311.5 release
description: Read about the known issues and fixed issues in Azure Local 2311.5 release.
author: ronmiab
ms.topic: conceptual
ms.date: 10/21/2024
ms.author: robess
...
# View known issues in Azure Local 2311.5 release

[!INCLUDE [applies-to](./includes/hci-applies-to-23h2.md)]

This article identifies the critical known issues and their workarounds in the Azure Local 2311.5 release.
...
> [!IMPORTANT]
> The production workloads are only supported on the Azure Local systems running the generally available 2311.2 release.
...
| Updates <!--27625941--> | When applying a cluster update to 10.2311.5.6 the `Get-SolutionUpdate` cmdlet may not respond and eventually fails with a RequestTimeoutException after approximately 10 minutes. This is likely to occur following an add or repair server scenario. | Use the `Start-ClusterGroup` and `Stop-ClusterGroup` cmdlets to restart the update service. </br><br> `Get-ClusterGroup -Name "Azure Local Update Service Cluster Group"` \| `Stop-ClusterGroup` </br><br> `Get-ClusterGroup -Name "Azure Local Update Service Cluster Group"` \| `Start-ClusterGroup` </br><br> A successful run of these cmdlets should bring the update service online. |
...
| Arc VM management <!--26066222--> |When a new server is added to an Azure Local instance, storage path isn't created automatically for the newly created volume.| You can manually create a storage path for any new volumes. For more information, see [Create a storage path](./manage/create-storage-path.md).  |
...
| Deployment |Deployments via Azure Resource Manager time out after 2 hours. Deployments that exceed 2 hours show up as failed in the resource group though the cluster is successfully created.| To monitor the deployment in the Azure portal, go to the Azure Local sresource and then go to new **Deployments** entry. |
