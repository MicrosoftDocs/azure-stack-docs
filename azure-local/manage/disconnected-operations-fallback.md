...
Appliance fallback logging is utilized to collect and send logs to Kusto, if the IRVM is down and standard log collection can't be initiated. This is useful in scenarios where the IRVM isn't accessible, but logs are still needed for troubleshooting.
...
Since the integrated runtime virtual machine (IRVM) isn't expected to work in the appliance fallback logging scenario, you need to shut it down to retrieve logs. To obtain the logs, mount and unlock the VHDs, then copy the logs from the mounted VHDs to a local, user-defined file location. You can configure the time window and roles that you want collected. If the `Observability Stamp ID` is configured, it should be included in the cmdlets return values.

Ensure there's enough space in this location to hold the logs, as the IRVM VHDs are temporarily mounted there during the copy action.
...
#### Copy from the IRVM and mount VHDs
...
3. Run the `Copy-DiagnosticData` cmdlet:
    ...
    > [!NOTE]
    > The IRVM is **restarted** after this operation is completed.
...
IRVM nested file structure:  

[![A screenshot of the IRVM nested file structure.](./media/disconnected-operations/fallback-log/nested-file-structure.png)](media/disconnected-operations/fallback-log/nested-file-structure.png#lightbox)
...
The `Get-ObservabilityStampId` cmdlet is a heavy operation and should only be needed in rare cases. This cmdlet also shuts down the IRVM and mounts the OS volume VHD to retrieve the stamp ID. After the operation is complete, the IRVM is **restarted**.
...
SYNOPSIS  
    Stops the IRVM, mounts required VHDs, retrieves the observability stamp ID (if available) and  
