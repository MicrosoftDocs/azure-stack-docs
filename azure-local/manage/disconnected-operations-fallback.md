...
Since the disconnected operations virtual machine isn't expected to work in the appliance fallback logging scenario, you need to shut it down to retrieve logs. To obtain the logs, mount and unlock the VHDs, then copy the logs from the mounted VHDs to a local, user-defined file location. You can configure the time window and roles that you want collected. If the `Observability Stamp ID` is configured, it should be included in the cmdlets return values.

Ensure there's enough space in this location to hold the logs, as the disconnected operations virtual machine VHDs are temporarily mounted there during the copy action.
...
#### Copy from the disconnected operations virtual machine and mount VHDs
...
3. Run the `Copy-DiagnosticData` cmdlet:
    ...
    > [!NOTE]
    > The disconnected operations virtual machine is **restarted** after this operation is completed.
...
[![A screenshot of the disconnected operations virtual machine nested file structure.](./media/disconnected-operations/fallback-log/nested-file-structure.png)](media/disconnected-operations/fallback-log/nested-file-structure.png#lightbox)
...
The `Get-ObservabilityStampId` cmdlet is a heavy operation and should only be needed in rare cases. This cmdlet also shuts down the disconnected operations virtual machine and mounts the OS volume VHD to retrieve the stamp ID. After the operation is complete, the disconnected operations virtual machine is **restarted**.
...
SYNOPSIS  
    Stops the disconnected operations virtual machine, mounts required VHDs, retrieves the observability stamp ID (if available) and  
