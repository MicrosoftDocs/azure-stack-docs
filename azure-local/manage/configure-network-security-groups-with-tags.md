...
By using network security tags, you can create custom user-defined tags, attach those tags to your virtual machine (VM) network interfaces, and apply network access policies (by using network security groups) based on these tags.
...
Network security groups let you configure access policies based on network constructs like network prefixes and subnets. For example, if you want to restrict communication between your web server VMs and database VMs, you must identify corresponding network subnets and create a policy to deny communication between those subnets. However, this approach has some limitations:

- Your security policies tie to network constructs, which means you need to know which applications reside on specific network segments. Understanding your network infrastructure and architecture becomes crucial.

- When building policies for applications, you might want to reuse them across different scenarios. For example, if your production web app can only be reached over port 80 from the internet, and can't be reached by other apps in production or other environments, you'd have a similar policy for any new app. However, with network segmentation, you need to recreate policies due to unique network elements for each app.

- If you decommission an old application and deploy a new one within the same network segment, you need to adjust policies.

By using network security tags, you no longer need to track the network segments where your applications are hosted. Network security tags simplify policy management and avoid the complexities associated with network constructs. Let's reconsider the example with web server and database VMs: Tag the corresponding VMs with **Web** and **Database** network security tags, and then create a rule to restrict communication between **Web** and **Database** tags.
...
1. On the Windows Admin Center home screen, under **All connections**, select the cluster where you want to create the network security group.

1. Under **Tools**, scroll down to the **Networking** section, and select **Network security groups**.

1. Under **Network security groups**, select the **Network Security Tags** tab, and then select **New**.
...
1. (Optional) In the **Type** field, enter a type for the tag. This field is useful if you want to categorize tags for easy management. For example, you can have different tags with the same type "Application", such as SQL, Web, IOT, Sensor, and so on.
...
1. On the Windows Admin Center home screen, under **All connections**, select the server or cluster where you want to create the VM.
...
Assign a network security tag to an existing VM by changing its settings. For detailed instructions, see [Change VM settings](vm.md#change-vm-settings).
...
1. Under the **Network Security Tag** section, select **Add Network Security Tag**, and select the network security tag you created earlier in [Create network security tag](#create-network-security-tags).
...
1. In the **Network security rule** pane on the right, provide the following information:
    ...
    | **Actions** | If the preceding conditions are matched, specify either to allow or block the packet. Acceptable values are **Allow** and **Deny**. |
...
To apply a network security group to a network security tag through Windows Admin Center, follow these steps:

1. On the Windows Admin Center home screen, under **All connections**, select the cluster where you want to apply the network security group.

1. Under **Tools**, scroll down to the **Networking** section, and select **Network security groups**.

1. Under **Network security groups**, select the **Network Security Tags** tab.
