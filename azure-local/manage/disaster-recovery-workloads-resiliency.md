...
Azure Local VMs can host high performance instances of [Arc-enabled SQL Server](/sql/sql-server/azure-arc/overview), including those using Always-On Availability Groups to enhance resiliency. The primary objective of SQL Server disaster recovery is to meet the application's specific Recovery Time Objective (RTO) and Recovery Point Objective (RPO) in the event of a disruption. SQL Server provides a rich set of built-in features for both high availability (HA) within a single site/cluster and disaster recovery to a secondary site/cluster.  
