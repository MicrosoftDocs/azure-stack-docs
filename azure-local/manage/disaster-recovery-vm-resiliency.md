...
## Storage path (volume) considerations
...
When deploying well-architected applications that utilize multiple Azure Local VMs to enhance workload resiliency, where possible it is advisable to store each VM's virtual hard disk (VHD/X) files in separate storage paths to optimize redundancy. For example, when provisioning multiple Azure Local VMs that each run instances of the same service, using different storage paths (that map to different volumes) helps mitigate the risk of all VMs (and instances of the application) relying on a single storage path (volume). The storage path used by Azure Local VMs is configured during deployment, when using infrastructure as code (IaC) to provision workloads, the placement logic employs a round-robin allocation logic for storage path selection, and in multi-node clusters, each storage path will map to different volumes, thus increasing resiliency if a single volume is unavailable or offline. For more granular provisioning control, the [--storage-path-id] parameter can be specified when using Azure CLI for workload deployment, allowing you to target specific storage paths when provisioning Azure Local VMs.
