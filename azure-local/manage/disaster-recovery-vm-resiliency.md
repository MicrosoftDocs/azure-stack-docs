...
When deploying well-architected applications that utilize multiple Azure Local VMs to enhance workload resiliency, where possible it is advisable to store each VM's virtual hard disk (VHD/X) files in separate storage paths to optimize redundancy. For example, when provisioning multiple Azure Local VMs that each run instances of the same service, using different storage paths (that map to different volumes) helps mitigate the risk of all instances relying on a single storage path and volume. The storage path used by Azure Local VMs is configured during deployment, when using infrastructure as code to provision workload, it will automatically employ a round-robin allocation strategy for storage paths (that map to volumes), which efficiently distributes the Azure Local VMs VHDs across different volumes in a multi-node cluster. For more granular provisioning control, the [--storage-path-id] parameter can be specified during Azure CLI deployment to target specific or alternative storage paths / volumes.
