...
When deploying well-architected applications that utilize multiple Azure Local VMs to enhance workload resiliency, where possible it is advisable to store each VM's virtual hard disk (VHD/X) files in separate storage paths to optimize redundancy. For example, when provisioning multiple Azure Local VMs that run instances of the same service, using different storage paths for the VMs VHDs helps mitigate the risk of all VMs VHDs using a single storage path (volume).
