...
By default, all virtual machines (VMs) on Azure Local are designed for high availability. If a physical node in the cluster fails, any VMs affected are automatically restarted and continue to operate on the remaining nodes, provided there is enough compute capacity. Still, even with these strong platform-level fault tolerance and resiliency features, it’s essential that the workloads and applications on your VMs follow well-architected principles.

Well-architected applications incorporate reliability into their code, infrastructure, and operations. For instance, running multiple instances of your workload services across two or more Azure Local VMs is crucial to eliminate single points of failure at the application layer. This setup helps reduce or prevent downtime if one application instance fails—see [workload resiliency](disaster-recovery-workloads-resiliency.md) for details. Additionally, well-designed applications should include business continuity and disaster recovery (BC/DR) strategies, such as routine backups and validation tests to confirm that disaster recovery procedures function correctly.

Review [Azure Well-Architected Framework (WAF) - Reliability design principles](/azure/well-architected/reliability/principles) for more information and recommendations to enhance the resiliency of your workload or application(s).

To mitigate the risk of service disruptions or data loss that cannot be mitigated by the redundancy offered by a single Azure Local instance, it is essential to implement a combination of comprehensive [backup strategies](#backup-tools) alongside continuous data replication technologies. Backup strategies protect against scenarios such as data corruption, accidental or malicious deletion, and catastrophic incidents, facilitating the restoration of data to a previous state when necessary. [Data replication technologies](#continuous-replication-of-business-critical-vms) enable synchronized copies of virtual machine data to be maintained across multiple Azure Local instances and/or Azure regions, which can minimize downtime and enable rapid workload failover in the event of hardware or system failure. Together, these approaches provide a robust safety net designed to safeguard data and sustain operational and business continuity during unexpected disruptions.

## Storage path (volumes) considerations

Azure Local instances deployed using the default "_express storage_" configuration, will have a 'UserStorage_X' cluster shared volumes (CSVs) created, one per physical node in the cluster, and an associated Storage Path resource created in Azure for each volume. The storage path resource in Azure is used by workload resources, such as Azure Local VMs, AKS Arc clusters and to store VM images. For example; a CSV with the local file system path of `C:\ClusterStorage\UserStorage_1\`, will have a storage path resource created in the Azure that represents the cluster shared volume (CSV) to enable Azure Local VMs virtual hard disks (VHD/Xs) to be placed on a specific target volume. For more information see [about storage paths](/azure/azure-local/manage/create-storage-path#about-storage-path).

When deploying well-architected applications that utilize multiple Azure Local VMs to enhance workload resiliency, where possible it is advisable to store each VM's virtual hard disk (VHD/X) files in separate storage paths to optimize redundancy. For example, when provisioning multiple Azure Local VMs that each run instances of the same service, using different storage paths (that map to different volumes) for the VMs VHDs helps mitigate the risk of all VMs (and instances of the application) relying on a single storage path (volume).

The storage path used by Azure Local VMs is configured when its virtual hard disk(s) are created. When using infrastructure as code (IaC) to provision workloads, the placement logic for VHDs employs a round-robin allocation logic for storage path selection. For multi-node Azure Local instances, by default each storage path will map to a different volume, therefore having the VMs VHDs distributed across multiple volumes increases workload resiliency, specifically if a single volume becomes temporarily unavailable or is offline for any reason. For more granular provisioning control, the [--storage-path-id] parameter can be specified when using Azure CLI for workload deployment, allowing you to target specific storage paths when provisioning Azure Local VMs.
