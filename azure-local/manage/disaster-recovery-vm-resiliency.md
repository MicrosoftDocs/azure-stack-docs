...
When deploying well-architected applications that utilize multiple Azure Local VMs to enhance workload resiliency, it is advisable to store each VM's virtual hard disk (VHD/X) files in separate storage paths to optimize redundancy. For instance, the VHDs for each Azure Local VM should be placed on distinct storage paths—different from those used by other VMs hosting instances of the same service—to mitigate the risk associated with relying on a single volume. During the creation of multiple Azure Local VMs in a deployment, infrastructure as code automation can employ a round-robin allocation strategy for storage paths, efficiently distributing workload VHDs across various volumes. For more granular provisioning control, the [--storage-path-id] parameter may be specified during deployment to target specific or alternative storage paths and volumes.
