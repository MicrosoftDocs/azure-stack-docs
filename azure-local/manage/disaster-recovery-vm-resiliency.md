...
When deploying well-architected applications use have multiple Azure Local VMs to provide increased workload resiliency, it is recommended that each VMs virtual hard disk drives (VHD/Xs) files are stored using a different storage paths to maximize resiliency. For example, the VHDs of each Azure Local VM should be deployed using a different storage path (compared to other VMs that host instances the same service), to help prevent a situation where all Azure Local VMs hosting instances of the same application or service are using a single storage path, that maps to a single volume. When creating multiple Azure Local VMs in a single deployment, using infrastructure as code automation, storage paths are selected using round-robin to evenly distribute workload VHDs across volumes. If you would like granular control when provisioning Azure Local VMs, it is possible to specify the [--storage-path-id] parameter during deployment to target a specific / different storage paths and volume.
