---
title: Fiber Channel disaggregated pattern without backup network
description: Plan to deploy an Azure Local disaggregated cluster using Fiber Channel SAN without a backup network.
ms.topic: how-to
author: alkohli
ms.author: cedward
ms.reviewer: alkohli
...
# Disaggregated Fiber Channel pattern without backup network for Azure Local
...
- There is **no requirement** for a dedicated in-guest backup network for virtual machine backup traffic.
- The cluster spans one or more racks with up to 64 nodes.

If your deployment requires a dedicated backup network for virtual machine backup traffic, see the [Fiber Channel pattern with backup network](fiber-channel-with-backup-disaggregated-pattern.md) instead.
...
Each server node connects to the network through four network adapter ports. Each server has a dual-port motherboard Open Compute Project (OCP) network adapter and a dual-port Peripheral Component Interconnect Express (PCIe) network adapter.

In this pattern:

- The **OCP dual-port adapter** is used for the management and compute intent.
- The **PCIe dual-port adapter** is used for the cluster networks.
...
| 17-32 | Cluster network PCIe ports | VLAN 1711 for cluster network 1. | VLAN 1712 for cluster network 2. | PCIe port 1 for cluster network 1. PCIe port 2 for cluster network 2. Both VLANs in access mode. |
| 33-48 | Available for additional network intents | N/A | N/A | Not used in this pattern. |
...
> [!NOTE]
> In a Fiber Channel (FC) SAN configuration, storage traffic runs entirely on the Fiber Channel fabric, separate from the Ethernet network. No storage Quality of Service (QoS) policies are needed on the Ethernet switches.
...
The following diagram shows a logical representation of how the host networking and Fiber Channel adapters are configured in this pattern:
