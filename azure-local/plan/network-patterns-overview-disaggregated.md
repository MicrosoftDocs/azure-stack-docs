...
A disaggregated deployment consists of single-node or multinode systems (up to 64 nodes per cluster) with the following characteristics:

> [!NOTE]
> Not all disaggregated deployments span multiple racks. If the disaggregated cluster remains single-rack and doesn't require SDN at scale, you can use the simpler single-rack topology with a two-switch HSRP pair.
...
- In iSCSI SAN configurations, storage traffic runs over dedicated Ethernet adapters and a dedicated iSCSI storage fabric.
...
## Supported disaggregated network reference patterns
...
| iSCSI SAN | Optional in-guest backup network (added post-deployment) | [iSCSI 6-NIC disaggregated pattern](iscsi-6-network-adapters-disaggregated-pattern.md) |

## Why use a leaf-spine (Clos) topology?

A traditional two-switch design (one pair of Top of Rack switches) works for small clusters but doesn't scale well. At 64 nodes across multiple racks:

- **Port limits** - A single Top of Rack (TOR) switch has about 48 ports and can't connect 64 or more nodes.
- **Failure impact** - If one switch fails, half your cluster loses connectivity.
...
During a spine failure or maintenance window, the effective oversubscription can increase to **4:1**. During this period, workloads with heavy east-west (cross-rack) traffic might experience reduced throughput until full spine capacity is restored.
...
> [!NOTE]
> BGP unnumbered is the recommended option, but using /31 point-to-point subnets is also supported. Choose either method based on your operational preferences.

The underlay runs in the default Virtual Routing and Forwarding (VRF) instance (the global routing table), but it's treated as infrastructure-only - it carries only loopback addresses and BGP peer routes. No default route (0.0.0.0/0) is injected into the underlay, and no workload prefixes are leaked into it. All workload traffic is isolated in dedicated overlay VRFs, each mapped to its own VXLAN Network Identifier (VNI). The underlay never sees tenant or cluster IP addresses - they're encapsulated inside VXLAN tunnels before entering the fabric.

This separation is the critical security boundary. If a misconfiguration or rogue route advertisement occurs in one overlay VRF, it can't affect the underlay or other VRFs - they're fundamentally separated by VXLAN encapsulation.
...
> [!IMPORTANT]
> Route leaking introduces shared reachability between otherwise isolated VRFs. Only leak the specific prefixes required for AKS-to-management communication. Avoid leaking default routes or broad summaries, as this action undermines the isolation benefits of separate VRFs.
...
| | Priority Flow Control (PFC) capability optional (not required for iSCSI) |
| **iSCSI storage** | iSCSI VLAN support (tagged 802.1Q) and static-routing capability for storage traffic isolation |
| | Multipath I/O (MPIO) compatible (no Link Aggregation Control Protocol (LACP) required on iSCSI links) |
