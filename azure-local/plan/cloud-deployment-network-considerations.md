...
> [!IMPORTANT]
> For switched hyperconverged deployments, we recommend running storage, management, and compute traffic over the same MLAG-configured top-of-rack (ToR) switch pair. A separate, isolated switch pair dedicated to the storage intent is not recommended, because it can create asymmetric failures in which nodes retain management connectivity but lose the storage east-west path, or the reverse, potentially resulting in a cluster split-brain condition. If this occurs, make recovery of the storage path the immediate operational priority.
