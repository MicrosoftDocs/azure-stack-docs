---
title: "Azure Local multi-rack security concepts"
description: Security overview for Azure Local multi-rack deployments.
author: sipastak
ms.author: sipastak
...
There are enhanced security options that let you protect your on-premises host servers as well as the Kubernetes clusters that run your Azure Local multi-rack workloads. These options are described below.
...
On-premises Kubernetes clusters that run your Azure Local multi-rack workloads are protected when you elect to enable the Microsoft Defender for Containers solution. [Microsoft Defender for Containers](/azure/defender-for-cloud/defender-for-containers-introduction) provides run-time threat protection for clusters and Linux nodes as well as cluster environment hardening against misconfigurations.
...
The Azure Local multi-rack Cluster Manager is an AKS implementation. The following image shows the Kube-Bench exceptions for the Cluster Manager. A full report of CIS Benchmark control evaluation for Azure Kubernetes Service (AKS) can be found [here](/azure/aks/cis-kubernetes)
...
Azure Local multi-rack deployments provide persistent storage for virtualized and containerized workloads. Data is stored and encrypted at rest on the storage appliances in the Azure Local aggregator rack. For more information, please see [the storage appliance reference documentation](/azure/operator-nexus/reference-near-edge-storage).
