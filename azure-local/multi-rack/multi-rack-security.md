...
There are enhanced security options that let you protect your on-premises host servers as well as the Kubernetes clusters that run your workloads. These options are described below.
...
Multi-rack deployments provide persistent storage for virtualized and containerized workloads. Data is stored and encrypted at rest on the storage appliances in the aggregator rack.

Multi-rack Kubernetes clusters and multi-rack VMs consume storage from a local disk. Data stored on local disks is encrypted using LUKS2 with the AES256 bit algorithm in XTS mode. All encryption keys are platform managed.