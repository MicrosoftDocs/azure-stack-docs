...
- You have details of your proxy server to provide during VM creation. Azure Local VMs don't have external connectivity to enable guest management without proxy details configured at the time of creation.
...
Skip this section if not using Azure CLI to provision and manage Azure Local VMs and VM resources.
...
- The latest version of Azure CLI. You must install this version on the client that you're using to connect to your Azure Local.

- For installation instructions, see [Install Azure CLI](/cli/azure/install-azure-cli-windows). Once you have installed Azure CLI, make sure to restart the terminal to ensure the `PATH` variable is set properly.
