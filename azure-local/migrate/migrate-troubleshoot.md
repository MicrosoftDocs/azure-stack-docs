...
To correct the display issue in Azure portal and reflect the true data disk size, follow the steps at [Expand a data disk](/manage/manage-arc-virtual-machine-resources.md#expand-a-data-disk) to reapply the same size as the current data disk (no actual size increase is required).
