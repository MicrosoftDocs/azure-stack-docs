...
# Resolve Azure Local migration issues when using Azure Migrate

## Summary

Azure Local migration issues can interrupt virtual machine (VM) moves from VMware when you use Azure Migrate. This article explains common causes and solutions so that you can resolve failures faster.

## Static IP address conflicts during migration

When you migrate a VM from VMware to Azure Local, and you retain a static IP address, the migration fails and generates an `InUse` error if the IP address is already assigned to another resource. This problem is common when you migrate domain controller VMs or other role-based VMs.
