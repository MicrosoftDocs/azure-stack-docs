...
1. Update the cluster API server endpoint to use the provisioned machine IP address.
   ...
   The kubeconfig generated by K3s uses a local loopback address that only works from inside the provisioned machine. Replace it with the machine's reachable IP address on your local network while retaining the port number after the colon (:).

   Example:

    ```bash
    kubectl config set-cluster default --server=https://<IP_ADDRESS>:<PORT>
    ```
