...
1. Sign in to your registry from the terminal:
   ...
   Replace `YOUR_REGISTRY_NAME` with your registry name. You can find the login server name on the **Overview** page of the registry in the Azure portal.
...
To remotely install open source K3s, connect the cluster to Azure Arc, and configure Azure RBAC, use [setup-k3s-arc.sh](/docs/scripts/setup-k3s-arc.sh). Otherwise, follow the steps below.
...
1. Create a kubeconfig file that the signed-in user can access:
...
1. Copy the kubeconfig output. You'll use it on your Windows PC.
