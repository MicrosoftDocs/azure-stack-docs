...
|---|---|---|---|
| Subscription setup | New required resource providers were added since the last release. | Deployment can fail if the required providers aren't registered. | [Validate your subscription against the current required provider list before deployment](https://learn.microsoft.com/azure/azure-local/small-form-factor/small-form-factor-subscription-setup?view=azloc-2606#register-the-required-resource-providers). |
...
| Retesting | TPM slot exhaustion can happen after repeated deployments, usually after about 15 runs. | Retesting can fail when no TPM slots remain. | [Manually clear the TPM in BIOS before you retest.](https://learn.microsoft.com/azure/azure-local/small-form-factor/small-form-factor-troubleshoot?view=azloc-2606#tpm-isnt-writable-during-provisioning) |
