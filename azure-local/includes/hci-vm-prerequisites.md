...
- Access to an Azure subscription with the appropriate Azure role-based access control (RBAC) role and permissions assigned. For more information, see [RBAC roles for Azure Local VM management](../manage/assign-vm-rbac-roles.md#about-built-in-rbac-roles).
- Access to a resource group where you want to provision the VM.
- Access to one or more VM images on your Azure Local instance. These VM images can be created by using one of the following procedures:
