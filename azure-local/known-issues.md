...
For the 2609 release of Azure Local, Microsoft released the following update:

| Solution version | OS build |
| -- | -- |
| 12.2609.1003.7 | 26100.33438 |

> [!IMPORTANT]
> New deployments of this software use the **12.2609.1003.7** build.
...
| Update <!--36808734--> | During the update, health checks might report the error: `SBE manifest endpoint not reported by Get-SolutionDiscoveryDiagnosticInfo`. | This error is at the warning level. You can ignore it while running the update. |
...
| Update | There's an intermittent issue in this release when the Azure portal incorrectly reports the update status as **Failed to update** or **In progress** though the update is complete. | [Connect to your Azure Local instance](./update/update-via-powershell-23h2.md#connect-to-your-azure-local) via a remote PowerShell session. To confirm the update status, run the following PowerShell cmdlets: <br><br> `$Update = get-solutionupdate`\| `? version -eq "<version string>"`<br><br>Replace the version string with the version you're running. For example, "10.2405.0.23". <br><br>`$Update.state`<br><br>If the update status is **Installed**, no further action is required. The Azure portal refreshes the status correctly within 24 hours. <br> To refresh the status sooner, follow these steps on one of the nodes. <br>Restart the Cloud Management cluster group.<br>`Stop-ClusterGroup "Cloud Management"`<br>`Start-ClusterGroup "Cloud Management"` |
...
| Deployment <!--33390832--> | In rare instances, deployment fails with errors during validation that state that the mandatory Arc extensions aren't yet installed. | If you face this issue, retry the deployment. |
| Security | If the Windows Defender attack surface reduction rule Block Process Creations originating from PSExec & WMI commands is configured to Block, the Azure Local Solution Update fails to run. | For detailed steps on how to resolve this issue, see the [Troubleshooting guide](https://github.com/Azure/AzureLocal-Supportability/blob/main/TSG/Update/Solution-Update-CAU-Run-fails-due-to-Windows-Defender-blocking-WMI-commands.md). |
...
## Known issues for version 2608

For the 2608 release of Azure Local, Microsoft released the following update:

| Solution version | OS build |
| -- | -- |
| 12.2608.1003.9 | 26100.33296 |
...
| Feature | Issue | Comments | Solution version |
| ------ | ------ | ------- | -------- |
| Update <!--39353802--> | A solution update could require an extra Cluster-Aware Updating (CAU) attempt to finish and might fail after using up its available retries. | Fixed an issue where a solution update could require an extra CAU attempt to finish and fail after exhausting its retries. | 12.2608.1003.9 |
...
The following table lists the known and expected system behaviors that aren't bugs or limitations.
...
## Known issues for version 2607

For the 2607 release of Azure Local, Microsoft released the following update:
...
| Feature | Issue | Comments | Solution version |
| ------ | ------ | ------- | -------- |
| Update <!--39016090--> | During solution updates on Storage Area Network (SAN)-only deployed clusters, the `UpdateStorage` step could run unexpectedly and block the update. | Fixed an issue where the `UpdateStorage` step could run during solution updates on SAN-only deployed clusters. | 12.2607.1003.73 |
| Update <!--39028679--> | Solution Builder Extension (SBE) update download fails. | Fixed SBE download logic to ignore an invalid extra download response object following an otherwise successful download. | 12.2607.1003.73 |
| Update <!--39044132--> | OS Cluster-Aware Updating (CAU) solution updates could fail with the error: `Prior Attempt Count: {3} is greater than Max Allowed Retries {2}`. | Fixed an issue that could cause OS CAU solution updates to fail due to incorrect retry count validation. | 12.2607.1003.73 |
| Update <!--38857483--> | Solution update preparation could fail with a `mismatch between update components and content` error when a child component version was already installed. | Fixed an issue where update preparation could fail when a child component version was already installed, resulting in a component and content mismatch error. | 12.2607.1003.73 |
...
| Azure Local VMs <!--38576202--> | If a networking step failed while the system was reconciling a VM, the VM's underlying cluster resource could be removed, putting the VM's availability at risk. | The system now protects the VM's cluster resource when a networking step fails, preserving VM availability. | 12.2607.1003.69 |
| Azure Local VMs <!--37712361--> | On large environments (up to 64 nodes), an internal timing issue could crash core VM agents, leading to VM management disruptions and instability. | The underlying timing issue was resolved, eliminating the agent crashes and improving stability at scale. | 12.2607.1003.69 |
| Azure Local VMs <!--37539073--> | A regression caused the capacity check that runs before VM operations to report incorrect results, which could block valid actions or allow ones that should be prevented. | Capacity check now reports correct results again. | 12.2607.1003.69 |
| Azure Local VMs <!--38090451--> | After upgrading, customers whose existing logical network IP ranges overlapped with a network created automatically during upgrade could not delete or modify network interfaces, seeing an "IP Pool ranges already "InUse" error. | Network interface changes now work even when customer IP ranges overlap the upgrade-created network, so the error no longer blocks the operation. | 12.2607.1003.69 |
| Azure Local VMs <!--37925682--> | After updating to 2603 or later, the DNS servers configured on a logical network no longer appeared in the Azure portal/CLI, even though they were still applied and working underneath. | Azure portal/CLI now displays the configured DNS servers correctly again. | 12.2607.1003.69 |
| Azure Local VMs <!--37873838--> | The logical network definition exposed obsolete internal status fields that were no longer used and could cause confusion. | The obsolete fields were removed to keep the logical network definition clean and current. | 12.2607.1003.69 |
| Azure Local VMs <!--37495079--> | In very large environments (2,000+ VMs), a monitoring component repeatedly crashed and restarted because it was allotted too little memory, which could disrupt metrics/monitoring. | The component's memory allocation was increased so it runs reliably at large scale. | 12.2607.1003.69 |
| Azure Local VMs <!--37710306--> | In very large environments (2,000+ VMs), a core management component was repeatedly shut down for exceeding its memory limit, disrupting VM management operations. | The component's memory limit was raised so it stays healthy at large scale. | 12.2607.1003.69 |
| Azure Local VMs <!--37710305--> | In very large environments (2,000+ VMs), a management component produced excessive logging and repeated no-op updates, adding unnecessary load to the system. | The redundant activity was eliminated, reducing system load at large scale. | 12.2607.1003.69 |
| Azure Local VMs <!--37710307--> | In very large environments (2,000+ VMs), the internal configuration database could run out of space, which could disrupt cluster operations. | Database sizing and cleanup were adjusted to support high VM counts without running out of space. | 12.2607.1003.69 |
| Azure Local VMs <!--37287964--> | A version mismatch between internal deployment components could cause Arc resource bridge deployment to hang and eventually time out after about two hours. | The components are now aligned, so deployment completes successfully without the timeout. | 12.2607.1003.69 |
...
For the 2606 release of Azure Local, Microsoft released the following update:

| Solution version | OS build |
| -- | -- |
| 12.2606.1003.205 | 26100.32995 |
...
| Azure Local VMs <!--36746514--> | When changing startup memory and vCPU outside of Azure, it reconciles these values but sometimes reverts to the old configuration and reboots unexpectedly. | Startup memory and vCPU changes outside of Azure are synced back to Azure and don't revert unexpectedly. This fix doesn't add support for syncing dynamic memory min/max values changed outside of Azure. |
...
For the 2605 release of Azure Local, Microsoft released the following update:

| Solution version | OS build |
|--|--|
| 12.2605.1003.1003 | 26100.32860 |

> [!IMPORTANT]
> New deployments of this software use the **12.2605.1003.1003** build.
The release notes for this version include the issues fixed in this release, known issues in this release, and known issues carried over from previous versions.
...
| Update <!--37373738--> | SBE pre-deployment health checks aren't executed if SBE version is 5.0 or higher. | |
| Azure Local VMs <!--35957329--> | In some Azure Local VM create scenarios, when you don't specify a storage path, the precheck incorrectly targets the infrastructure volume instead of an eligible customer storage path. | The precheck logic is updated to correctly resolve and target the appropriate customer storage path when you don't explicitly provide a storage path, preventing the infrastructure volume from being used unintentionally. |
| Azure Local VMs <!--33489182--> | In some cases, gallery image create or update operations take longer than expected or timeout with larger images. | Gallery image create and update operations are optimized to improve reliability and performance, resulting in faster and more consistent image operations. |
| Azure Local VMs <!--34513309--> | VMConnect couldn't connect to Azure Local virtual machines if you deployed them in a different resource group than the Azure Local instance. | VMConnect now supports connectivity to Azure Local VMs across different resource groups from the Azure Local instance. |
| Update <!--33993719--> | Cloud Management service fails because the subscription isn't active. | An update health check was added to validate the subscription state is active. |
| Update <!-- 35851395--> | Azure Local doesn't discover SBE update. | An update health check was added to validate the discovery URL is matching hardware model and SKU. |
| Deployment <!--36020508--> | Fixed an issue that can cause deployment to fail because of DNS timeout. | |
| Update <!--35943750--> | Changed the logic to ensure the last SBE update isn't cleaned up from the update history when it's older than 180 days. | |
| Update <!--34603698--> | Fixed an issue that could prevent resuming an SBE update run. | |
| Update <!--36009293--> | Fixed an issue that could cause the update service to crash when sideloading an update. | |
| Update <!--36305565--> | Fixed an issue that prevents installing an update when a prior one failed in health check step. | |
| Repair server <!--32447442--> | Fixed an issue where `Add-server` and `Repair-server` cmdlets may fail with the error:`Cluster Build ID matches node to add's Build ID`. | |
...
| Feature | Issue | Workaround |
| --------- | --------- | --------- |
| Azure Local VMs <!--37983058--> | Instances of Azure Local version 2601 and later might experience unintended virtual machine deletion when a platform component incorrectly classifies virtual machines during routine system operations. | To protect your Azure Local instances from potential unintended VM deletion, update the Microsoft On-Premises Cloud (MOC) component by using the [Remediation Support Tool for Azure Local infrastructure component issues](/azure/azure-local/manage/remediate-support-tool-infrastructure?view=azloc-2604&preserve-view=true). Once the MOC component update is applied, the Azure Local instance is no longer exposed to this issue. For the latest update, see [Release information summary](release-information-23h2.md). |
| Azure Local VMs <!--30775126--> | The `az stack-hci-vm stop` command fails when using CLI version 1.14.x against clusters running versions prior to 2604. | Use the latest CLI and make sure to include `--skip-shutdown` parameter to force a turn off (`az stack-hci-vm stop --skip-shutdown`). Alternatively, downgrade to an earlier CLI version (1.13.0 or earlier) where the default stop behavior is supported on all clusters. |
...
| Update | When you view the readiness check results for an Azure Local instance via the Azure Update Manager, there might be multiple readiness checks with the same name. | There's no known workaround in this release. Select **View details** to view specific information about the readiness check. |
| Update | There's an intermittent issue in this release when the Azure portal incorrectly reports the update status as **Failed to update** or **In progress** though the update is complete. | [Connect to your Azure Local instance](./update/update-via-powershell-23h2.md#connect-to-your-azure-local) via a remote PowerShell session. To confirm the update status, run the following PowerShell cmdlets: <br><br> `$Update = get-solutionupdate`\| `? version -eq "<version string>"`<br><br>Replace the version string with the version you're running. For example, "10.2405.0.23". <br><br>`$Update.state`<br><br>If the update status is **Installed**, no further action is required. The Azure portal refreshes the status correctly within 24 hours. <br> To refresh the status sooner, follow these steps on one of the nodes. <br>Restart the Cloud Management cluster group.<br>`Stop-ClusterGroup "Cloud Management"`<br>`Start-ClusterGroup "Cloud Management"` |
| Add server <!--26852600--> | In this release and previous releases, when adding a machine to the system, you can't update the proxy bypass list string to include the new machine. Updating environment variables proxy bypass list on the hosts doesn't update the proxy bypass list on Azure resource bridge or AKS. | There's no workaround in this release. If you encounter this issue, contact Microsoft Support to determine next steps. |
| Azure portal <!--25741164--> | In some instances, the Azure portal might take a while to update and the view might not be current. | You might need to wait for 30 minutes or more to see the updated view. |
| Security <!--30348397--> | Azure Local might face an issue during normal operations (for example, Update, Repair) while using Defender for Endpoint and when the **Restrict App Execution** setting is enabled for one or more servers in the deployment. | Disable the **Restrict App Execution** setting in the Defender portal and reboot. If the issue persists, [open a support case](/azure/azure-portal/supportability/how-to-create-azure-support-request). |
| Deployment <!--33390832--> | In rare instances, deployment fails with errors during validation that state that the mandatory Arc extensions aren't yet installed. | If you face this issue, retry the deployment. |
| Security | If the Windows Defender attack surface reduction rule Block Process Creations originating from PSExec & WMI commands is configured to Block, the Azure Local Solution Update fails to run. | For detailed steps on how to resolve this issue, see the [Troubleshooting guide](https://github.com/Azure/AzureLocal-Supportability/blob/main/TSG/Update/Solution-Update-CAU-Run-fails-due-to-Windows-Defender-blocking-WMI-commands.md). |
...
> [!IMPORTANT]
> New deployments of this software use the **12.2602.1002.501** build.
The release notes for this version include the issues fixed in this release, known issues in this release, and known issues carried over from previous versions.
...
> [!IMPORTANT]
> New deployments of this software use the **12.2601.1002.503** build.
The release notes for this version include the issues fixed in this release, known issues in this release, and known issues carried over from previous versions.
...
| Feature | Issue | Comments |
| --------- | --------- | --------- |
| Update <!--35747709--> | Update may fail when the cloud management group runs on a different node than the owner node with the error: `Type 'RegisterCloudManagementUpdatesExtension' of Role 'CloudManagementConfig' raised an exception: Exception occurred in Get-ClusterExtension'` | |
...
| Deployment <!--36579554--> | When using three or more intents in the system, PowerShell `Invoke-Command` call crashes with the error: `Type 'SetAzureStackHostsPreConfiguration' of Role 'HostNetwork' raised an exception: Cannot get intent status of Storage. Exception: No intent statuses found for intent Storage.` | |
...
> [!IMPORTANT]
> New deployments of this software use the **12.2512.1002.16** build.
...
> [!IMPORTANT]
> New deployments of this software use the **12.2511.1002.502** build.
Release notes for this version include the issues fixed in this release, known issues in this release, and release note issues carried over from previous versions.
...
| Update | When installing cumulative updates using Azure Update Manager, only the latest update for version 2507 is installed. If earlier update options (for versions 2505, 2506) are selected, they aren't installed. | |
| Update <!--34867064--> | Update health check results aren't shown when it's been longer than three hours after health check completed. | |
...
> [!IMPORTANT]
> New deployments of this software use the **12.2510.1002.531** build. You can also update an existing deployment from 2509 by using **11.2510.1002.93**.
...
| Security <!-- 56969147 --> | When fixing the compliance for the minimum password length rule, even after you change the minimum password length on the Azure Local host to 14, you continue to see it as noncompliant in Azure policy. | |
| Azure Local VMs <!-- 30323252--> | Azure Local VMs have empty DVD drives attached after deployment. | Newly created Azure Local VMs no longer include empty DVD drives after deployment. |
| Azure Local VMs <!--30856036--> | In some cases, Azure Local VMs fail to create with error: `Failed to cleanup seed iso disk from the file system for vm.` | This build addresses additional corner cases that weren't resolved in the previous fix (2503). |
| Azure Local VMs <!--33648132--> | In rare cases, the operator enters a crash loop, preventing Azure Local VM management. | |
| Azure Local VMs <!--34444839--> | In some cases, deleting logical networks fails when network interfaces created with those logical networks existed in other resource groups or locations. | Updated deletion logic to ensure safe deletion of logical network. |
...
| Add server <!--26852600--> | In this release and previous releases, when adding a machine to the system, you can't update the proxy bypass list string to include the new machine. Updating environment variables proxy bypass list on the hosts doesn't update the proxy bypass list on Azure resource bridge or AKS. | There's no workaround in this release. If you encounter this issue, contact Microsoft Support to determine next steps. |
| Azure portal <!--25741164--> | In some instances, the Azure portal might take a while to update and the view might not be current. | You might need to wait for 30 minutes or more to see the updated view. |
| Update | When updating the Azure Local instance via the Azure Update Manager, the update progress and results may not be visible in the Azure portal. | To work around this issue, on each node, add the following registry key (no value needed):<br><br>`New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\Services\HciCloudManagementSvc\Parameters" -force`</br><br> Then on one of the nodes, restart the Cloud Management cluster group. </br><br>`Stop-ClusterGroup "Cloud Management"`</br><br>`Start-ClusterGroup "Cloud Management"`</br><br> This action doesn't fully fix the issue as the progress details may still not be displayed for a duration of the update process. To get the latest update details, you can [Retrieve the update progress with PowerShell](./update/update-via-powershell-23h2.md#step-6-track-update-progress). |
| Security <!--30348397--> | Azure Local might face an issue during normal operations (for example, Update, Repair) while using Defender for Endpoint and when the **Restrict App Execution** setting is enabled for one or more servers in the deployment. | Disable the **Restrict App Execution** setting in the Defender portal and reboot. If the issue persists, [open a support case](/azure/azure-portal/supportability/how-to-create-azure-support-request). |
| Deployment <!--33471589--> | After Azure portal deployment, SConfig network settings shows the error: `Set-SCfNetworksetting : Cannot bind argument to parameter 'Value' because it is null.` | There's no known workaround in this release. |
| Deployment <!--33390832--> | In rare instances, deployment fails with errors during validation that state that the mandatory Arc extensions aren't yet installed. | If you face this issue, retry the deployment. |
| Update | When you install cumulative updates by using Azure Update Manager, only the latest update for version 2507 is installed. If you select earlier update options (for versions 2505, 2506), the updates aren't installed. | There's no workaround in this release. |
| Update <!--34867064--> | Update health check results aren't shown when it's been longer than three hours after health check completed | Rerun the update health checks to restart the three hour expiration clock. |
...
> [!IMPORTANT]
> New deployments of this software use the **12.2509.1001.22** build. You can also update an existing deployment from 2508 by using **11.2509.1001.21**.
...
| Feature | Issue | Comments |
| --------- | --------- | --------- |
| Azure Local VMs, Azure Verification <!--58937961--> | VMs on Azure Local running Windows Server Azure Edition, Windows 10, or Windows 11 multi-session OS may not activate properly. A pop-up message or a watermark may display, indicating that Windows isn't activated. The VM functions, but the watermark persists. | VMs are now activated properly. |
| Azure Local VMs <!--30573455--> | Recreating an image with the same name may fail due to remnants of a previous failed deployment, occasionally resulting in the creation of a zero-byte image. | Automatic proper cleanup of failed image artifacts, allowing subsequent image creation attempts to succeed. |
| Deployment <!--34633094--> | Disk types aren't determined correctly, resulting in false negative results on storage pool size determination. | Improved validation for resolving disk type when determining storage pool size. |
...
| Azure portal <!--25741164--> | In some instances, the Azure portal might take a while to update and the view might not be current. | You might need to wait for 30 minutes or more to see the updated view. |
| Update | When updating the Azure Local instance via the Azure Update Manager, the update progress and results may not be visible in the Azure portal. | To work around this issue, on each node, add the following registry key (no value needed):<br><br>`New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\Services\HciCloudManagementSvc\Parameters" -force`</br><br> Then on one of the nodes, restart the Cloud Management cluster group. </br><br>`Stop-ClusterGroup "Cloud Management"`</br><br>`Start-ClusterGroup "Cloud Management"`</br><br> This action doesn't fully fix the issue as the progress details may still not be displayed for a duration of the update process. To get the latest update details, you can [Retrieve the update progress with PowerShell](./update/update-via-powershell-23h2.md#step-6-track-update-progress). |
| Security <!-- 56969147 --> | When fixing the compliance for the minimum password length rule, even after you change the minimum password length on the Azure Local host to 14, you continue to see it as noncompliant in Azure policy. | You can verify the length of the password by using the `net accounts` cmdlet. In the output, find **Minimum password length** to see the value. |
| Security <!--30348397--> | Azure Local might face an issue during normal operations (for example, Update, Repair) while using Defender for Endpoint and when the **Restrict App Execution** setting is enabled for one or more servers in the deployment. | Disable the **Restrict App Execution** setting in the Defender portal and reboot. If the issue persists, [open a support case](/azure/azure-portal/supportability/how-to-create-azure-support-request). |
| Deployment <!--33471589--> | After Azure portal deployment, SConfig network settings shows the error: `Set-SCfNetworksetting : Cannot bind argument to parameter 'Value' because it is null.` | There's no known workaround in this release. |
| Deployment <!--33390832--> | In rare instances, deployment fails with errors during validation that state that the mandatory Arc extensions aren't yet installed. | If you face this issue, retry the deployment. |
| Update | When you install cumulative updates by using Azure Update Manager, only the latest update for version 2507 is installed. If you select earlier update options (for versions 2505, 2506), the updates aren't installed. | There's no workaround in this release. |
...
|Feature  |Issue  |Comments  |
|---------|---------|---------|
| Azure Local VMs <!--33528074--> | For Disconnected operations with Azure Local VMs, the Arc agent download fails. | Corrected endpoint and now Arc agent download succeeds. |
...
| Deployment <!--33731727--> | Disk types weren't determined correctly, resulting in false negative results on storage pool size determination. |  |
...
| Feature | Issue | Workaround |
| --------- | --------- | --------- |
| Azure Verification <!--58937961--> | VMs on Azure Local running Windows Server Azure Edition, Windows 10, or Windows 11 multi-session OS may not activate properly. A pop-up message or a watermark may display, indicating that Windows isn't activated. The VM functions, but the watermark persists. | There's no known workaround in this release. |
| Add server <br> Repair server <!--32447442--> | The `Add-server` and `Repair-server` cmdlets fail with the error: <br> `Cluster Build ID matches node to add's Build ID`. | Use the OS image of the same solution version as the version running on the existing cluster. To get the OS image, identify and download the image version from this [Release table](https://github.com/Azure-Samples/AzureLocal/blob/main/os-image/os-image-tracking-table.md). |
| Deployment <!--33008717--> | In this release and previous releases, registration fails with the following error when you try to register Azure Local machines with Azure Arc: <br>`AZCMAgent command failed with error: >> exitcode: 42. Additional Info: See https://aka.ms/arc/azcmerror`. | For detailed steps on how to resolve this issue, see the [Troubleshooting guide](https://github.com/Azure/AzureLocal-Supportability/blob/main/TSG/ArcRegistration/TSG-Arc-registration-failing-with-error-42.md). |
| Azure Local VM management | The Mochostagent service might appear to be running but can get stuck without updating logs for over a month. You can identify this issue by checking the service logs in `C:\programdata\mochostagent\logs` to see if logs are being updated. | Run the following command to restart the mochostagent service: `restart-service mochostagent`. |
| Update | When you view the readiness check results for an Azure Local instance via the Azure Update Manager, there might be multiple readiness checks with the same name. | There's no known workaround in this release. Select **View details** to view specific information about the readiness check. |
...
| Azure portal <!--25741164--> | In some instances, the Azure portal might take a while to update and the view might not be current. | You might need to wait for 30 minutes or more to see the updated view. |
| Update | When updating the Azure Local instance via the Azure Update Manager, the update progress and results may not be visible in the Azure portal. | To work around this issue, on each node, add the following registry key (no value needed):<br><br>`New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\Services\HciCloudManagementSvc\Parameters" -force`</br><br> Then on one of the nodes, restart the Cloud Management cluster group. </br><br>`Stop-ClusterGroup "Cloud Management"`</br><br>`Start-ClusterGroup "Cloud Management"`</br><br> This action doesn't fully fix the issue as the progress details may still not be displayed for a duration of the update process. To get the latest update details, you can [Retrieve the update progress with PowerShell](./update/update-via-powershell-23h2.md#step-6-track-update-progress). |
| Security <!-- 56969147 --> | When fixing the compliance for the minimum password length rule, even after you change the minimum password length on the Azure Local host to 14, you continue to see it as non-compliant in Azure policy. | You can verify the length of the password by using the `net accounts` cmdlet. In the output, find **Minimum password length** to see the value. |
| Security <!--30348397--> | Azure Local might face an issue during normal operations (for example, Update, Repair) while using Defender for Endpoint and when the **Restrict App Execution** setting is enabled for one or more servers in the deployment. | Disable the **Restrict App Execution** setting in the Defender portal and reboot. If the issue persists, [open a support case](/azure/azure-portal/supportability/how-to-create-azure-support-request). |
| Deployment <!--33471589--> | After Azure portal deployment, SConfig network settings shows the error: `Set-SCfNetworksetting : Cannot bind argument to parameter 'Value' because it is null.` | There's no known workaround in this release. |
| Deployment <!--33390832--> | In rare instances, deployment fails with errors during validation that state that the mandatory Arc extensions aren't yet installed. | If you face this issue, retry the deployment. |
| Update | When you install cumulative updates by using Azure Update Manager, only the latest update for version 2507 is installed. If you select earlier update options (for versions 2505, 2506), the updates aren't installed. | There's no workaround in this release. |
...
The following issue is known for this release.
...
> [!IMPORTANT]
> New deployments of this software use the **12.2506.1001.29** build. You can also update an existing deployment from 2505 by using **11.2506.1001.28**.
...
| ------ | ------ | ---------- |
| Azure Local VMs <!--30035765--> | When creating a VM, there's no option to prompt for password. | Password is prompted when `--password` isn't explicitly passed. This change provides customers a more secure way to input password. |
...
| Azure Local VMs <!--32417179--> | In some cases, VM update operations were incorrectly shown as successful on the Azure portal even though they failed in the underlying infrastructure. | Improved error reporting to ensure the Azure portal accurately reflects the true status of VM update operations. |
| Azure Local VMs <!--32503885--> | Deleting a logical network with an attached AKS cluster left behind orphaned on-premises resources. | Properly detects and handles AKS cluster dependencies during logical network deletion to prevent resource leaks. |
...
| Azure Local VMs <!--32195527--> | In some cases, deleting a VM from multiple platforms (Azure portal, Windows Admin Center, Failover Cluster Manager) caused its high availability (HA) status to remain stuck in a pending state. | Correctly updates the HA status when the VM is removed from the cluster, preventing it from getting stuck. |
...
| Azure Local VMs <!--30846971--> | In some cases, VM deletions didn't properly clean up associated network interfaces or disks, leaving behind orphaned resources that blocked reuse of IP addresses and storage. | All dependent resources are correctly detached and deleted when a VM is deleted, preventing resource leaks. |
...
| Update | When updating the Azure Local instance via the Azure Update Manager, the update progress and results may not be visible in the Azure portal. | To work around this issue, on each node, add the following registry key (no value needed):<br><br>`New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\Services\HciCloudManagementSvc\Parameters" -force`</br><br> Then on one of the nodes, restart the Cloud Management cluster group. </br><br>`Stop-ClusterGroup "Cloud Management"`</br><br>`Start-ClusterGroup "Cloud Management"`</br><br> This action doesn't fully fix the issue as the progress details may still not be displayed for a duration of the update process. To get the latest update details, you can [Retrieve the update progress with PowerShell](./update/update-via-powershell-23h2.md#step-6-track-update-progress). |
| Security <!-- 56969147 --> | When fixing the compliance for the minimum password length rule, even after you change the minimum password length on the Azure Local host to 14, you continue to see it as non-compliant in Azure policy. | You can verify the length of the password by using the `net accounts` cmdlet. In the output, find **Minimum password length** to see the value. |
| Upgrade <!--33417006--> | The upgrade banner is currently available for users using the Azure Government cloud. However, the environment checker fails, suggesting that Azure Government clouds aren't supported. | There's no workaround in this release. If you encounter this issue, contact Microsoft Support to determine next steps. |
...
|Feature  |Issue  |Workaround  |
|---------|---------|---------|
| Update <!--54889342--> |  A critical VM operational status not OK alert appears in the Azure portal under **Update readiness** and in the **Alerts** pane after the update finishes successfully. Additionally, the alert appears when running the `Get-HealthFault` cmdlet. | No action is required. This alert resolves automatically in a few days. |
| Deployment <!--33153622-->| Updating Azure Arc extensions manually from the Azure Local Machine page via the Azure portal results in issues during deployment. The extensions that you shouldn't update manually are: `AzureEdgeDeviceManagement`, `AzureEdgeLifecycleManager`, and `AzureEdgeAKVBackupForWindows`. | Installing extensions manually from the Azure Local machine page isn't supported. |
| Upgrade <!--33417006-->| The upgrade banner is currently available for users using the Azure Government cloud. However, the environment checker fails, suggesting that Azure Government clouds aren't supported. | There's no workaround in this release. If you encounter this issue, contact Microsoft Support to determine next steps. |
...
| Deployment <!--33008717--> | In this release and previous releases, registration fails with the following error when you try to register Azure Local machines with Azure Arc: <br>`AZCMAgent command failed with error: >> exitcode: 42. Additional Info: See https://aka.ms/arc/azcmerror`. | For detailed steps on how to resolve this issue, see the [Troubleshooting guide](https://github.com/Azure/AzureLocal-Supportability/blob/main/TSG/ArcRegistration/TSG-Arc-registration-failing-with-error-42.md). |
| Update | When you view the readiness check results for an Azure Local instance via the Azure Update Manager, there might be multiple readiness checks with the same name. | There's no known workaround in this release. Select **View details** to view specific information about the readiness check. |
| Update | There's an intermittent issue in this release where the Azure portal may incorrectly display the update status as **Failed to update** or **In progress**, even though the update actually completes successfully. This behavior is particularly observed when updating Azure Local instances via Azure Update Manager, where the update progress and results may not be visible in the portal. | You might need to wait up to 30 minutes or more to see the updated status. If the status still isn't refreshed after that time, follow these steps: [Connect to your Azure Local instance](./update/update-via-powershell-23h2.md#connect-to-your-azure-local) via a remote PowerShell session. To confirm the update status, run the following PowerShell cmdlets: <br><br> `$Update = get-solutionupdate`\| `? version -eq "<version string>"`<br><br>Replace the version string with the version you're running. For example, "10.2405.0.23". <br><br>`$Update.state`<br><br>If the update status is **Installed**, no further action is required. Azure portal refreshes the status correctly within 24 hours. <br> To refresh the status sooner, follow these steps on one of the nodes. <br>Restart the Cloud Management cluster group.<br>`Stop-ClusterGroup "Cloud Management"`<br>`Start-ClusterGroup "Cloud Management"` |
| Add server <!--26852600--> | In this release and previous releases, when adding a machine to the system, you can't update the proxy bypass list string to include the new machine. Updating environment variables proxy bypass list on the hosts doesn't update the proxy bypass list on Azure resource bridge or AKS. | There's no workaround in this release. If you encounter this issue, contact Microsoft Support to determine next steps. |
| Azure portal <!--25741164--> | In some instances, the Azure portal might take a while to update and the view might not be current. | You might need to wait for 30 minutes or more to see the updated view. |
| Security <!-- 56969147 --> | When fixing the compliance for the minimum password length rule, even after you change the minimum password length on the Azure Local host to 14, you continue to see it as non-compliant in Azure policy. | You can verify the length of the password by using the `net accounts` cmdlet. In the output, find **Minimum password length** to see the value. |
...
> [!IMPORTANT]
> The new deployments of this software use the **12.2504.1001.20** build. You can also update an existing deployment from 2503 by using **11.2504.1001.21**.
...
| Azure Local VMs <!--30353875--> | Issues with deleting and resizing an Azure Local VM that has checkpoint(s). | You can checkpoint an Azure Local VM with on-premises tools. The VM remains manageable from the Azure portal. |
...
| --------- | --------- | --------- |
| Add server <br> Repair server <!--32447442--> | The `Add-server` and `Repair-server` cmdlets fail with the error: <br> `Cluster Build ID matches node to add's Build ID`. | Use the OS image of the same solution version as the version running on the existing cluster. To get the OS image, identify and download the image version from this [Release table](https://github.com/Azure-Samples/AzureLocal/blob/main/os-image/os-image-tracking-table.md). |
| Add server <br> Repair server <!--32483959--> | If you run `Add-server` and `Repair-server` cmdlets with a customized storage adapter IP configured in your Azure Local instance, the operation might fail with the error: <br> `Type 'ConfigureAzureStackHostStorageAdpaterIPAddressesInAddRepairNode' of Role 'HostNetwork' raised an exception: Connecting to remote server <MACHINE> failed with the following error message : Access is denied.`. | Contact Microsoft Support if you experience this issue. |
...
| Deployment <!--33008717--> | In this release and previous releases, registration fails with the following error when you try to register Azure Local machines with Azure Arc: <br>`AZCMAgent command failed with error: >> exitcode: 42. Additional Info: See https://aka.ms/arc/azcmerror`. | For detailed steps on how to resolve this issue, see the [Troubleshooting guide](https://github.com/Azure/AzureLocal-Supportability/blob/main/TSG/ArcRegistration/TSG-Arc-registration-failing-with-error-42.md). |
| Azure Arc registration <!-- 32995193 --> | When registering a new machine with Azure Arc, registration fails during `ImageRecipeValidationTests` with the following error:<br>` "Responses": [ { "Name": "ImageRecipeValidation", "Status": "Failed", "Errors": [ { "ErrorMessage": "Diagnostics failed for the test category: ImageRecipeValidation.", "StackTrace": null, "ExceptionType": "DiagnosticsTestFailedException", "RecommendedActions": [ "Please contact Microsoft support." ] } ] } ` | For detailed steps on how to resolve this issue, see the [Troubleshooting guide](https://github.com/Azure/AzureLocal-Supportability/blob/main/TSG/ArcRegistration/TSG-Arc-registration-failing-with-error-42.md). |
| Deployment <!--642400448--> | In the 2504 release, when you run the **Install Azure Stack HCI** wizard and select a time zone, the system continues to use Pacific Standard Time (PST), regardless of your selection. | This isn't a blocking issue, and deployment still completes successfully. |
...
> [!IMPORTANT]
> New deployments of this software use the **2503.0.13** build. You can also update from 2411.3.
...
| Update <!--30232441--> | Ensure that Solution Builder Extension updates of type *Notify* that are imported get copied correctly before validation. | |
...
|Feature|Issue|Workaround/Comments|
|------|------|----------|
| Azure Local VMs <!--ADO--> | Can't delete an Azure Local VM enabled by Arc when one of the nodes is down.| Contact Microsoft Support if this issue occurs.|
...
| Microsoft Defender for Cloud <br><br> Azure Government <!--32555179-->| In the Azure Government cloud, Microsoft Defender for Cloud recommendations for Azure Local don't show up in the Microsoft Defender for Cloud portal.|  |
