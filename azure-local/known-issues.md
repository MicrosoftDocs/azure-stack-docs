...
|Azure Local VMs <!--37540842--> | Deleting a VM could fail with the error `An invalid IP address was specified`, leaving the VM and its network interface stuck and unable to be removed. |VM deletion now completes reliably, even for VMs whose network interface has no assigned private IP.|12.2607.1003.71|
|Azure Local VMs <!--37773461--> | When updating a VM through an older API or tool version, the VM’s placement settings could be silently cleared, which could affect where the VM is scheduled. | Placement settings are now preserved automatically during updates, so they are no longer lost. |12.2607.1003.71|
|Azure Local VMs <!--38576202--> | If a networking step failed while the system was reconciling a VM, the VM’s underlying cluster resource could be removed, putting the VM’s availability at risk. | The system now protects the VM’s cluster resource when a networking step fails, preserving VM availability. |12.2607.1003.71|
|Azure Local VMs <!--37712361--> | On large environments (up to 64 nodes), an internal timing issue could crash core VM agents, leading to VM management disruptions and instability. | The underlying timing issue was resolved, eliminating the agent crashes and improving stability at scale. |12.2607.1003.71|
|Azure Local VMs <!--37539073--> | A regression caused the capacity check that runs before VM operations to report incorrect results, which could block valid actions or allow ones that should be prevented. | Capacity check now reports correct results again. |12.2607.1003.71|
|Azure Local VMs <!--38090451--> | After upgrading, customers whose existing logical network IP ranges overlapped with a network created automatically during upgrade could not delete or modify network interfaces, seeing an “IP Pool ranges already “InUse” error. | Network interface changes now work even when customer IP ranges overlap the upgrade-created network, so the error no longer blocks the operation. |12.2607.1003.71|
|Azure Local VMs <!--37925682--> | After updating to 2603 or later, the DNS servers configured on a logical network no longer appeared in the Azure portal/CLI, even though they were still applied and working underneath. | Azure portal/CLI now displays the configured DNS servers correctly again. |12.2607.1003.71|
|Azure Local VMs <!--37873838--> | The logical network definition exposed obsolete internal status fields that were no longer used and could cause confusion. | The obsolete fields were removed to keep the logical network definition clean and current. |12.2607.1003.71|
|Azure Local VMs <!--37495079--> | In very large environments (2,000+ VMs), a monitoring component repeatedly crashed and restarted because it was allotted too little memory, which could disrupt metrics/monitoring. | The component’s memory allocation was increased so it runs reliably at large scale. |12.2607.1003.71|
|Azure Local VMs <!--37710306--> | In very large environments (2,000+ VMs), a core management component was repeatedly shut down for exceeding its memory limit, disrupting VM management operations. | The component’s memory limit was raised so it stays healthy at large scale. |12.2607.1003.71|
|Azure Local VMs <!--37710305--> | In very large environments (2,000+ VMs), a management component produced excessive logging and repeated no-op updates, adding unnecessary load to the system. | The redundant activity was eliminated, reducing system load at large scale. |12.2607.1003.71|
|Azure Local VMs <!--37710307--> | In very large environments (2,000+ VMs), the internal configuration database could run out of space, which could disrupt cluster operations. | Database sizing and cleanup were adjusted to support high VM counts without running out of space. |12.2607.1003.71|
|Azure Local VMs <!--37287964--> | A version mismatch between internal deployment components could cause Arc resource bridge deployment to hang and eventually time out after about two hours. | The components are now aligned, so deployment completes successfully without the timeout. |12.2607.1003.71|
