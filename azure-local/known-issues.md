...
| Azure Arc registration <!-- 32995193 --> | When registering a new machine with Azure Arc, registration fails during `ImageRecipeValidationTests` with the following error:<br>` "Responses": [ { "Name": "ImageRecipeValidation", "Status": "Failed", "Errors": [ { "ErrorMessage": "Diagnostics failed for the test category: ImageRecipeValidation.", "StackTrace": null, "ExceptionType": "DiagnosticsTestFailedException", "RecommendedActions": [ "Please contact Microsoft support." ] } ] } ` | Complete these steps:<br>1. Open PowerShell as Administrator.<br>2. Run the following command:<br>`Get-ScheduledTask -TaskName ImageCustomizationScheduledTask`<br>3. If the state is `Ready`, do the following:<br>a. Run `Get-ScheduledTask -TaskName ImageCustomizationScheduledTask \| Start-ScheduledTask`<br>b. Wait for the task to reach the `Disabled` state. Rerun the `Get-ScheduledTask -TaskName ImageCustomizationScheduledTask` command as needed until the task state shows `Disabled`. |
