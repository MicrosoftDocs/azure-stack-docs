...
A/B update extends commit workflow v2 with policy‑driven staged steps:<br>
1. **Lock (configuration)**: Freeze fabric configuration to batch the intended updates.<br>
1. **Prepare (start)**: Build candidate configurations based on policy (for example, stage‑customer edge‑configuration cycle).<br>
1. **Staged apply (continue to CE1/CE2)**: Apply to the selected customer edge device only. No other devices change.<br>
1. **Validate and test**: Run traffic/telemetry checks and device configuration views.<br>
1. **Cancel (rollback)**: Revert all devices to previous golden configuration if results aren't acceptable.<br>
1. **Complete**: Roll out to remaining devices, and then release the lock.<br>
...
- Commit cancel rollback restores the last known good configuration to all devices and Resource Manager resources. You can use this operation after staging the configuration on one device.
...
- An A/B update currently doesn't support staging for ToRs, network packet brokers, or management switches. Some layer 3 isolation domain operations can't be fully validated during customer edge staging.
- The A/B update and commit v2 workflows (patch and update) are mutually exclusive and can't be performed in parallel.
- After the A/B update workflow is initiated, the service doesn't support switching over to commit v2 workflow.
