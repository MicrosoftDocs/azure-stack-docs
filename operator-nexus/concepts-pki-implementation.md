...
- **Automatic rotation**: cert-manager renews certificates before they expire based on the `Certificate` resource settings (such as `duration` and `renewBefore`).
- **Default certificate lifetime**: Most Azure Operator Nexus leaf TLS certificates are issued for **70 days**.
