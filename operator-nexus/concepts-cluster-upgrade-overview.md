...
Before a compute rack upgrade starts, the platform cordons all provisioned and ready Bare Metal Machines (BMMs) in that rack. The rack upgrade doesn't proceed until the process cordons all eligible BMMs. Cordoning the entire rack first prevents workloads evacuated from one server from being scheduled onto another server in the same rack that is about to be upgraded. 
