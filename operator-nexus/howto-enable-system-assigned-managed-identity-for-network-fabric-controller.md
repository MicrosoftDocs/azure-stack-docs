...
# Enable a SAMI for the Network Fabric Controller in Azure Operator Nexus

This article describes how to enable and validate a system-assigned managed identity (SAMI) for the Network Fabric Controller resource for both new resources and existing resources.
...
Network Fabric Controller resource managed flows access platform resources (for example, Azure Storage and Azure Key Vault in managed resource groups). A SAMI is the required baseline identity for trusted service access patterns in these workflows.

## Prerequisites

- The Azure CLI is signed in to the target subscription.
...
## Create a Network Fabric Controller resource with a SAMI
...
| `--mi-system-assigned` | Enables a SAMI on create. No value required.                      | flag only     |
...
| `--mi-system-assigned` | Adds or retains a SAMI on the resource identity.                           | flag only   |
...
### Option B: A SAMI association via support personnel

If you can't use the direct Azure CLI update, or you require an automated support channel to perform a SAMI association, contact support personnel. Provide the following details:
...
Support personnel can perform a SAMI association operation on your behalf.

## Verification

Here's how to verify the identity and state of a SAMI.
...
|------------------|---------------------|-------|------------------|
| no identity      | add a SAMI          | Yes   | `SystemAssigned` |
| SystemAssigned   | add a SAMI          | Yes   | retained         |
| SystemAssigned   | add a UAMI          | No    | rejected         |
...
- Identity type is invalid.
  ...
  - Fix: Rerun with `--mi-system-assigned` only.
    ...
    > [!IMPORTANT]
    > If identity type `None` is submitted, the managed identity resource provider might remove the SAMI context before the Network Fabric resource provider can block the request. This condition leads to token acquisition failures in Network Fabric Controller resource managed platform flows. Reassociate the SAMI as soon as possible if this condition occurs.
...
*Why is a user-assigned managed identity (UAMI) not supported for the Network Fabric Controller resource?*

The Network Fabric Controller resource uses a SAMI as the required identity bound to its resource lifecycle. A UAMI isn't in scope for these managed flows.

*Can I remove a SAMI from the Network Fabric Controller resource?*

No. A SAMI must persist to maintain trusted service access to Network Fabric Controller resources (such as Key Vault and Storage in its managed resource group).
...
*How do I verify that a SAMI is properly active on the Network Fabric Controller resource?*
...
Configurations of an existing SAMI is unchanged unless identity fields are explicitly included in the payload.
