...
| NNF device disk space | Minimum 2.5 GB of free space within /mnt directory of all the network devices that are getting upgraded | No | Device upgrade command will fail for corresponding device |
...
Each of the NNF devices will enter maintenance mode post triggering the upgrade. Traffic is drained and route advertisements are stopped.
