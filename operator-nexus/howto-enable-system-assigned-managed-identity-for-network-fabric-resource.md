---
title: Enable SAMI for the Network Fabric Resource in Azure Operator Nexus
description: This article describes how to enable SAMI for the Network Fabric resource in Azure Operator Nexus. You can enable SAMI for new and existing resource paths.
...
- Azure CLI is signed in to the correct subscription and tenant.
...
The following sections show you different ways to create a new resource.

### Create a Network Fabric resource with SAMI only
...
For the full create command argument set, see [Create a Network Fabric](https://learn.microsoft.com/azure/operator-nexus/howto-configure-network-fabric#create-a-network-fabric).
...
### Create Network Fabric resource with UAMI only
...
When you use UAMI for storage access, also include `--storage-account-config` to link the UAMI to the storage account identity:
...
For more information, see [Bring Your Own Storage for Network Fabric](https://learn.microsoft.com/azure/operator-nexus/howto-configure-bring-your-own-storage-network-fabric).
...
When you configure UAMI for storage access in a SAMI and UAMI topology, include `--storage-account-config` to link the UAMI to the storage account identity. See the preceding section for details.

## Use an existing resource

The following sections show you different ways to work with an existing resource.

### Add SAMI when the Network Fabric resource has no identity
...
### Add SAMI when the Network Fabric resource already has UAMI
...
### Add UAMI when the Network Fabric resource already has SAMI
...
- Required order:
  - First, perform the identity patch and confirm the Network Fabric resource is in `PendingCommit`.
  - Second, acquire a lock.
  - Third, trigger a commit.
  - Fourth, verify a commit completion and the resulting state.
  - Fifth, unlock if the lock remains set after a failed or interrupted flow.
...
- Invalid identity combination.
  ...
  - Fix: Use a valid transition payload shape. Avoid `None`. Preserve SAMI when required.
...
*How do I add UAMI to a Network Fabric resource that already has SAMI?*  

Use `--mi-system-assigned --mi-user-assigned <uami-id>`. SAMI is preserved and UAMI is added.

*What identity should I use for Bring Your Own Storage scenarios?*  

Use UAMI. If SAMI is already present, specify both identities as `SystemAssigned,UserAssigned`. Include `--storage-account-config` to link the UAMI to the storage account.
...
Yes. UAMI is reusable, but ensure that the correct role assignments are configured for each resource the UAMI is attached to.
