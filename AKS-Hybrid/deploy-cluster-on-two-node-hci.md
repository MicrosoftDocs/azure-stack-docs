...
This article describes the recommended architecture for deploying an AKS hybrid cluster on a two-node Azure Stack HCI system.<!--Lead sentence isn't accurate. The article only describes failover and recovery scenarios--> 

Traditional Kubernetes deployments require three physical machines to survive a single failure. This usually means a higher Total Cost of Ownership (TCO) for the customer. For cost-sensitive deployments, AKS hybrid can be deployed on a two-node Azure Stack HCI system with a few trade-offs in availability.  
...
| user cluster | Kubernetes clusters that are deployed by Azure Kubernetes Service (AKS) to host the containerized end-user application or workload. |
...
| Existing user workload clusters | CRUD on user workload clusters | New user workload cluster lifecycle | API server availability |
|---------------------------------|-----------------------------------------|-------------------------------------|-------------------------|
...
| Existing workloads | CRUD on user workload clusters | New user workload cluster lifecycle | API server availability |
|--------------------|-----------------------------------------|---------------------------|-------------------------|
