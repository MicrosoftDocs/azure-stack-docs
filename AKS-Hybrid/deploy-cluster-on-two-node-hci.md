...
| If an application has a dependency on an external VIP of the API or liveness probe using the clusters VIP, there will be application downtime. |  Workloads can't be scaled while the load balancer VM is in a Failed state, and you can't add new worker nodes, scaling pods, and so forth. | Workload creation will succeed. | The API server remains unavailable while the load balancer VM is down. |
