...
| Failover Clustering | Failover Clustering for Azure Stack HCI provides infrastructure features that support high availability of VMs and applications. If a cluster node or service fails, the VMs or applications that are hosted on that node can be automatically or manually transferred to another available node in a process known as failover. For more information, see [Failover Clustering](/windows-server/failover-clustering/failover-clustering-overview). |
...
| If the worker nodes are on the same physical host, Failover Clustering will fail over the worker nodes on the surviving host.<br><br>If the application wasn't created with anti-affinity, Kerberos will move the pod over to the existing worker node.<br><br>If the application depends on the API server, and the control plane VM or load balancer VM of the workload cluster goes down, Failover Clustering will move those VMs to the active host<!--Correct terminology in a failover cluster would be the "secondary node." To avoid defining terminology, let's use "active" (instead of "surviving," since a failed host isn't necessarily a dead one.)-->, and the application will resume working. These pods may restart because of the way the application handles the loss of the API server. | Failover Clustering will fail over the control plane VM of the user’s cluster to the healthy host. Once this is done, scaling workloads will work. | Failover Clustering will fail over the control plane VM of the management cluster on the healthy host. Once this is done, CRUD operations on new target clusters will work. | Failover Clustering will fail over the control plane VM of the users cluster on the healthy host. Once this is done, the API server will be available. |
...
| If the worker nodes are on the same physical host, Failover Clustering fails over the worker nodes on the surviving host.<br><br>If the application wasn't created with anti-affinity, Kerberos will move the pod over to the existing worker node.<br><br>If the application has a dependency on the API server, and the control plane VM or load balancer VM of the user’s cluster shuts down, Failover Clustering moves those VMs to the surviving host, and the application will resume working. Depending on how the application handles the loss of the API server, pods may be restarted on the new host. | Failover Clustering fails over the control plane VM of the user’s cluster on the healthy host. After failover, workloads can be scaled. | Failover Clustering fails over the control plane VM of the management cluster on the healthy host. After failover, CRUD operations on new target clusters will work. | Failover Clustering restarts the control plane VM of the user’s cluster on the healthy host. After that, the API server is available. |
