...
## Networking
This guide assumes that you have control over your network and router (that is, in a home setting). If you are in a corporate environment, we recommend you ask your network administrator for a range of free IP addresses (from the same subnet) that 's reachable on the internet, then follow the guide below. 
...
| **Service IP range** | If `ServiceIPRangeSize` parameter is defined, will start at *192.168.0.4* | Both `ServiceIPRangeStart` and `ServiceIPRangeSize` parameters can be defined | 
...
During single machine deployment, AKS Edge Essential will create an internal virtual switch named `aksedgesw-int` and the appropriate virtual network adapters to connect the virtual machines and the Windows host OS. The setup will also handle the IP address assignation for the NICs and the creation of the NAT table. IP addresses are always defined, and are the following:
...
Finally, by using the NAT component, traffic is able to reach the external network using the physical network adapter. The diagram above shows an external network using the *10.0.0.0/24* IP address range; however this will depend on the networking environment that the device is installed.

## Scalable cluster

When running a scalable cluster, the different nodes running insdie the different devices must communicate between each other. Becuase of this, all nodes must be connected to the same network, so AKS EE deployment will use external virtual switches to manage the networking. The following diagram, shows a single machine deployment architecture using internal virtual switch.
...
### DNS
Domain Name System (DNS) translates human-readable domain names (for example, www.microsoft.com) to machine-readable IP addresses (for example, 192.0.2.44). The AKS Edge Essetianls Linux virtual machine uses [*systemd*](https://systemd.io/) (system and service manager), so the DNS or name resolution services are provided to local applications and services via the [systemd-resolved](https://www.man7.org/linux/man-pages/man8/systemd-resolved.service.8.html) service. The Windows node uses defualt Windows netowrking DNS service.

By default, during AKS EE deployment, the user can provide the DNS Servers list using the `DnsServers` parameter. If no address is provided, the deploymnet mechanism will look for the Windows host OS DNS servers (check using `ipconfing /all`) and use those server addresses. 

It's possible to check the DNS servers being used for both Linux and Windows nodes. To do so, you using an elevated PowerShell session run the following cmdlet: 
...
- For Windows VM node
    ...
    ```
    The command's output will show a list of the Windows VM network interfaces configured. In particular, it's important to check the *Ethernet adapter vEthernet (Ethernet)* interface status, which will be the default interface for the AKS EE VM communication. Also, make sure to check the IP addresses of the **DNS Servers**  field of the list. If there's no IP address, or the IP address isn't a valid DNS server IP address, then the DNS service won't work.
