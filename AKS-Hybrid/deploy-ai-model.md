...
1. The following details from your infrastructure administrator:
   ...
   - Make sure the AKS Arc cluster runs on the Azure local cluster with a supported GPU model. You must also identify the right GPU VM SKUs based on the model before creating the node pool. For more information, see [use GPU for compute-intensive workloads](deploy-gpu-node-pool.md).
   - We recommend using a computer running with Linux for this feature.
...
You need to have a running AKS Arc cluster with a default node pool. To deploy the KAITO operator, follow the steps below:
...
Before adding a GPU node, please make sure Azure Local has enabled with a supported GPU model and the GPU drivers are installed on all the host nodes. To create a GPU node pool using the Azure portal or Azure CLI, follow these steps:
...
1. Under **Settings** and **Node pools**, select **Add**. During the preview, we only support Linux node. Fill in the other required fields and create the node pool.
...
To create a GPU node pool using Azure CLI, run the following command. The GPU VM SKU used in the following example is for A16 model; for the full list of VM SKUs, see [Supported VM sizes](deploy-gpu-node-pool.md#supported-vm-sizes).
...
After the node pool creation succeeds, you can confirm whether the GPU node is provisioned using `kubectl get nodes`. In the following example, the GPU node is **moc-l1i9uh0ksne** and the other node is from the default node pool created during cluster creation:
...
1. Create a YAML file using the following template. KAITO supports popular OSS models such as Falcon, Phi3, Llama2, and Mistral. This list might increase over time.
   ...
   - We recommend using `labelSelector` and `preferredNodes` to explicitly select the GPU node by name. In the following example, `app: llm-inference` is used for the GPU node `moc-le4aoguwyd9`. You can choose any node label you want as long as the label matches, the next step shows how to label the node.
...
1. Validate the workspace using `kubectl get workspace` command. Also make sure that both `ResourceReady` and `InferenceReady` fields are set to **True** before testing with the prompt.
...
If the pod is not deployed properly or **ResourceReady** field shows empty or **false**, it's usually because the preferred GPU node isn't labeled correctly. Check the node label with `kubectl get node <yourNodeName> --show-labels`. For example, in the YAML file, the following code specifies that the node must have the label `apps=llm-inference`:
