...
| `Network.Ip4AddressPrefix` |Subnet mask `A.B.C.0/24`|The subnet from which IP addresses will be alloted in the case of single machine deployments.  For example, if your IP address prefix range is **192.168.1.0/24**, then the Gateway address will be 192.168.1.1, Linux node will be 192.168.1.2, Windows node would be 192.168.1.3 and the Service IPs would be assigned 192.168.1.4 onwards. | |Single machine deployment only|
