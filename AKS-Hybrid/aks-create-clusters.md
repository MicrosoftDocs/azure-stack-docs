...
Use the `az akshybrid create` command to create an AKS Arc cluster. Make sure you login to Azure before running this command. If you have multiple Azure subscriptions, select the appropriate subscription ID using the [az account set](/cli/azure/account#az-account-set) command.
...
Note that you need Contributor permissions on the resource group that hosts the AKS hybrid cluster in order to run the following command successfully:
