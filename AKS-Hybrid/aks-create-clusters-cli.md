...
- Before you begin, make sure you have the following details from your on-premises infrastructure administrator:
  ...
  - **Network ID** - Azure Resource Manager ID of the Azure AKS Arc vnet created following [these steps](aks-networks). Your admin should give you the ID of the AKS Arc vnet. This parameter is required in order to create Kubernetes clusters. You can also get the Azure Resource Manager ID using `az akshybrid vnet show --name "<vnet name>" --resource-group <azure resource group> --query "id" -o tsv` if you know the resource group in which the Azure Arc vnet was created.
