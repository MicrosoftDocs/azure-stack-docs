---
title: Deploy a Kubernetes (AKS) cluster using an ARM template
...
# Quickstart: Deploy a Kubernetes cluster using an ARM template
...
1. Verify the connection to your cluster using the `kubectl get` command. This command returns a list of the cluster nodes:
   ...
   ```
## [Optional] Step 7: Deploy Nodepool using an ARM template
You may also deploy the nodepool resource using ARM template. Please refer to the template below and save locally as azuredeploy.json
...
## [Optional] Step 8: Deploy the template and validate the results using Az CLI
Review the template and apply. It will take a few minutes to complete. You can use Az CLI to validate the node pool is created successfully. 
