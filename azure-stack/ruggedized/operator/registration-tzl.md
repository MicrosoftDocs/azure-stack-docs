...
- The subscription ID for an Azure subscription.  
    ...
    > [!Note]  
    > Azure subscriptions are associated to Azure cloud environments (../../operator/azure commercial, Azure government etc.) This determines which cloud you will connect to access Marketplace content.
    > 
...
- **Connected**: Connected means you've deployed Azure Stack Hub so that it can connect to the internet and to Azure. You can either have Azure AD or Active Directory Federation Services (../../operator/aD FS) for your identity store.
...
> [!NOTE]
> The error message **[InvalidRegistrationToken]:BillingModel 'Custom' is not supported by subscription** indicates that the subscription you are using for registration has not been approved for Azure Stack Hub ruggedized or MDC use. Please contact azshregistration@microsoft.com to have a subscription approved along with the type of product (../../operator/azure Stack Hub ruggedized or MDC) you are registering.
