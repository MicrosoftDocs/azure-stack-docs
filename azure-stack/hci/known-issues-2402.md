...
| Security <!--26865704--> |For new deployments, Secured-core capable devices will not have Dynamic Root of Measurement (DRTM) enabled by default. If you try to enable (DRTM) using the Enable-AzSSecurity cmdlet, you will see an error that DRTM setting is not supported in the current release.<br> Microsoft recommends defense in depth, and UEFI Secure Boot still protects the components in the Static Root of Trust (SRT) boot chain by ensuring that they are loaded only when they are signed and verified.  |DRTM is not supported in this release.|
