...
| Security <!--26865704--> |For new deployments, Secured-core capable devices will not have Dynamic Root of Measurement (DRTM) enabled by default. If you try to enable (DRTM) using the Enable-AzSSecurity cmdlet, you will see an error that DRTM setting is not supported in the current release.  |DRTM is not supported in this release.|
