...
|Feature|Issue|Workaround/Comments|
|------|------|----------|
| Repair-Server <!--27053788--> |In rare instances, the `Repair-Server` operation fails with the `HealthServiceWaitForDriveFW` error. In these cases, the old drives from the repaired node are not removed and new disks are stuck in the maintenance mode.    |There is no workaround for this issue. If you encounter this issue, contact Microsoft Support for next steps. |
