...
| Deployment <!--25955031--> |Deployment may fail with this error: Exception while installing ARB in step [DeployArb:Calling Install-ArcHciMgmt]. This error is owing to a known issue with the cache not getting cleaned up for Managed Service Identity.  |First try to [Rerun the deployment](./deploy//deploy-via-portal.md#rerun-deployment). If that doesn't work, delete the resource group along with the failed cluster. Redo the deployment. For more information, see [Deploy via Azure portal](./deploy/deploy-via-portal.md).   |
| Azure portal <!--26100617--> |In this release, there is an issue due to which the deployment or update fails if service principal secret has a hyphen as starting character.|Remove or move the hyphen from the service principal secret to a different location.   |
