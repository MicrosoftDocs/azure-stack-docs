...
|2|Host networking |Updating jumbo packet configuration on existing network intents has no effect. |Make sure to configure overrides to default values during the network intent creation. After your cluster is deployed, you can create a network intent that uses a customized jumbo packet. <br> If the cluster is in WDAC enforcement mode, switch the node from where you'll set the override in `Audit` mode. To switch the local node to audit, run the following command: <br> `Enable-ASLocalWDACPolicy -Mode Audit` <br> To create a new compute intent `NewComputeIntent` on adapters `NIC1` and `NIC2` that use JumboPacket as 9014, run the following commands:<br> `$adapterOverrides = New-NetIntentAdapterPropertyOverrides`<br><br>`$adapterOverrides.JumboPacket = 9014`<br><br>`Add-NetIntent -Name NewComputeIntent -Compute -AdapterName @("NIC1", "NIC2") -AdapterPropertyOverrides $adapterOverrides`|
...
|#|Feature|Issue|Workaround|
|-|------|------|----------|
|1|Deployment |If incorrect domain credentials are provided in the **Join a domain** page or using the *config file*, the deployment fails. You need to [Reset the deployment](./deploy/deployment-tool-troubleshoot.md#reset-deployment).|There's no workaround in this release. Make sure that the domain credentials that you provide are correct.<br><br>This behavior changes in the upcoming release and a workaround is available.|
...
|15|Deployment |CONFIRM DELETION- The **Region** and **Management traffic virtual switch** parameters in the **Services** page in the deployment tool aren't used in the deployment.|This issue will likely be fixed in a future release. |
