...
|3|Host networking |After the Azure Stack HCI cluster deployment, if you try to create a new network ATC compute intent with overrides, the operation would fail with the following error: *Cannot set property. Property setting is supported only on core types in this language mode*.|Property setting is not allowed in Constrained Language mode. ASK: Provide steps to work around this problem.  |
