...
|3|Host networking |After the Azure Stack HCI cluster deployment, if you try to create a new network ATC compute intent with override, the operation would fail with the following error: *Cannot set property. Property setting is supported only on core types in this language mode*.|CONFIRM - Should we switch to Audit mode here as well so that WDAC won't block this operation? |
