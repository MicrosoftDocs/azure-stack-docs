...
|2|Networking |Using `Set-NetIntent` to create a network intent with a customized jumbo packet network setting doesn't work correctly. |After your cluster is deployed, you can create a network intent that uses a customized jumbo packet for the adapter. <br> To create a new compute intent `NewComputeIntent` on adapters `NIC1` and `NIC2` that use JumboPacket as 9014, run the following commands: `$adapterOverrides = New-NetIntentAdapterPropertyOverrides`<br><br>`$adapterOverrides.JumboPacket = 9014`<br><br>`Add-NetIntent -Name NewComputeIntent -Compute -AdapterName @("NIC1", "NIC2") -AdapterPropertyOverrides $adapterOverrides`
 <br> For more information, see [Troubleshoot Azure Stack HCI registration issues and errors](./deploy/troubleshoot-hci-registration.md).|
