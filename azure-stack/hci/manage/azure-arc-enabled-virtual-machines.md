...
- Arc Resource Bridge provisioning through CLI should be performed on a local HCI server PowerShell. It cannot be done in a remote PowerShell window from a machine which is not a host of the Azure Stack HCI cluster. To connect on each nodes of Azure Stack HCI cluster use RDP connected with a domain user admin of the cluster.
