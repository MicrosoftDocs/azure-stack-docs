...
If the Failover Clustering feature isn't installed, install it on each server in the cluster with the `Install-WindowsFeature` cmdlet, using the -IncludeAllSubFeature and -IncludeManagementTools parameters:
...
If there's a critical security update that you need to apply quickly, or you need to ensure that updates complete within your maintenance window, this method may be for you. This process brings down the Azure Stack HCI cluster, updates the servers, and brings it all up again.
...
In older versions of Windows Admin Center, you may be prompted to authenticate multiple times during an updating run. Either authenticate each time when prompted, or go back to **Connections** and reconnect to the cluster.
...
This CredSSP error is seen when Windows Admin Center is running on a local PC and when the Windows Admin Center user isn't the same user who installed Windows Admin Center on the machine.
