...
This topic provides instructions for configuring access control lists (ACLs) to manage data traffic flow using Datacenter Firewall for Software-Defined Networking (SDN) in Azure Stack HCI using Windows PowerShell. You enable and configure Datacenter Firewall by creating ACLs that get applied to a virtual subnet or a network interface.
