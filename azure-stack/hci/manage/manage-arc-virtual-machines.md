...
1. On the **Overview** page for the VM, from the top command bar in the right pane, select **Delete**, then select **Yes**. 
    ...
    You are now prompted to confirm the deletion. Select **Yes**. 
...
1. You can now go to the resource group where this VM was deployed. You can see that the VM is removed from the list of resources in the resource group. You may need to select the option to **Show hidden types** to view the resources associated with this VM that were not deleted. Locate the associated resources such as the network interfaces and data disks, and delete them.
