...
- A minimum of one server and a maximum of 16 servers.

-  Each server must be from the same manufacturer, same model, and have the same number and type of drives.
...
- Have direct-attached drives that are physically attached to one server each.
...
> [!IMPORTANT]
> **NOT SUPPORTED**: RAID controller cards or SAN (Fibre Channel, iSCSI, FCoE) storage, shared SAS enclosures connected to multiple servers, or any form of multi-path IO (MPIO) where drives are accessible by multiple paths.
