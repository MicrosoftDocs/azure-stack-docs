...
|Deployment <!--x--> |EdgeRemoteSupport extension shows Failed on the cluster view, but Succeeded on the individual node view. | In the Azure portal, navigate to the resource group for your nodes and do the following:</br><br> 1. Go to individual Arc nodes.</br><br> 2. Uninstall the Remote Support extension. </br><br> 3. Wait for the Azure Stack HCI Resource Provider (RP) to reconcile the extensions. This can take up to 12 hours. </br><br> 4. If you don't want to wait you can trigger a force sync from any of the cluster nodes using the following cmdlet: `sync-azurestackhci`. </br><br> This process allows the extension to be reinstalled and the failures on the cluster view should go away.
