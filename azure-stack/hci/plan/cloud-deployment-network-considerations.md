...
The following diagram shows the various decisions and steps that define the network design framework for your Azure Local system - cluster size, cluster storage connectivity, network traffic intents, management connectivity, and network adapter configuration. Each design decision enables or permits the design options available in subsequent steps:
...
To help determine the size of your Azure Local system, use the [Azure Local sizer tool](https://azurestackhcisolutions.azure.microsoft.com/#/sizer), where you can define your profile such as number of virtual machines (VMs), size of the VMs, and the workload use of the VMs such as Azure Virtual Desktop, SQL Server, or AKS.

As described in the Azure Local [system server requirements](../concepts/system-requirements-23h2.md#server-and-storage-requirements) article, the maximum number of servers supported on Azure Local system is 16. Once you complete your workload capacity planning, you should have a good understanding of the number of server nodes required to run workloads on your infrastructure.

- **If your workloads require four or more nodes**: You can't deploy and use a switchless configuration for storage network traffic. You need to include a physical switch with support for Remote Direct Memory Access (RDMA) to handle storage traffic. For more information on Azure Local cluster network architecture, see [Network reference patterns overview](./network-patterns-overview.md).
...
When doing the initial deployment of your Azure Local system, you must define an IP range of consecutive IPs for infrastructure services deployed by default. 
...
For Azure Local system, you have two options to assign IPs for the server nodes and for the cluster IP. 
