...
The following table lists network attributes for deployments with microsegmentation and QoS enabled:
...
|HNV (AKA PA network)|2 IPs for each host<br><br>**Single node**:<br>1 SLB VM IP<br>1 GW VM IP|N/A|Requires default gateway to route packets externally.|Provider Address Network VLAN.<br>Subnet needs to allocate hosts and SLB VMs.<br>Potential subnet growth consideration.|IPs automatically assigned out of the subnet by NC|
