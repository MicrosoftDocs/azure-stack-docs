...
- To install updates using Windows Admin Center, PowerShell, or the Server Configuration tool (SConfig), see [Updating single-node clusters](../deploy/single-server.md#updating-single-node-clusters). For solution updates (such as driver and firmware updates), see your solution vendor.
