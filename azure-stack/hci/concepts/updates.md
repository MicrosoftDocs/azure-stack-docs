...
> [!NOTE]
> Deploying annual feature updates by using Windows Server Update Services (WSUS) is currently not supported. You can use Windows Update or download the [Azure Stack HCI operating system ISO file](../deploy/download-operating-system.md) to perform a rolling cluster upgrade via Cluster-Aware Updating (CAU), or manually update each node.
