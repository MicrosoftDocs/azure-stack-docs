...
4.  If the VM is **OK**, then evaluate the DVM. If the DVM has an error message:
    ...
    - The public key might be invalid. Review the key that you created.  
    - You need to contact your Azure Stack administrator to retrieve the logs for Azure Stack by using the privileged endpoints. For more information, see [Azure Stack diagnostics tools](../operator/azure-stack-diagnostics.md).
