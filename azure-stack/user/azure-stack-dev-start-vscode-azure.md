...
description: Set up the Azure Account extension in Visual Studio Code to connect to Azure Stack Hub. Follow step-by-step instructions for authentication and Cloud Shell access.
author: sethmanheim
ms.topic: how-to
ms.date: 07/08/2026
ms.author: sethm
...
1. Run the following cmdlets in the same session:
...
1. Make a note of the tenant ID. You need it when adding the JSON section that configures the Azure Account extension.

## Set up the Azure Account extension

1. Open VS Code.

1. Select **Extensions** on the left-side corner.

1. In the search box, enter `Azure Account`.

1. Select **Azure Account** and select **Install**.
...
1. In the code editor, update the following JSON snippet with the values for your environment, and then paste snippet into the settings block.
...
1. To test that you successfully logged into your Azure Stack Hub subscription, use **Ctrl**+**Shift**+**P** and select **Azure: Select Subscription** and see if the subscription you have is available.
...
| Azure: Create an Account | If you don't have an Azure Account, you can [sign up](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn) for one today. |
