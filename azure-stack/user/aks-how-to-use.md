...
1.  Ask your ASH administrator to install [ASH Update 2107](https://docs.microsoft.com/en-us/azure-stack/operator/release-notes?view=azs-2102) or greater. Azure Kubernetes Service in Azure Stack Hub Preview is only available starting with that update
2.  Ask your ASH administrator to create a plan and offer that includes this resource provider, see the [installation instructions document](https://microsoft.sharepoint.com/:w:/t/AKSandACRonASHPrivatePreview/EcurzWgNWulBuMsiU-vz_-ABUkBoGZlYQsT8tqwSeYWXKQ?e=4h4xub).
...
2.  Connect to your Azure Stack Hub endpoint. You need to tell Azure CLI to which Azure Stack Hub you are connecting to. Follow the instructions [here](https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-version-profiles-azurecli2?view=azs-2005#connect-to-azure-stack-hub) to:
...
1.  Sign into the environment.
    ...
    > [!NOTE]  
    > If you get a "certificate verify failed" error, it may be the case that the cert used for the Azure Resource Manager endpoint is not trusted by your client machine. If so you need to export the cert used in Azure Stack Hub end points and trust it, follow the instructions here: [https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-version-profiles-azurecli2?view=azs-2005\#export-the-azure-stack-hub-ca-root-certificate](https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-version-profiles-azurecli2?view=azs-2005#export-the-azure-stack-hub-ca-root-certificate).   
      
    In particular, for Linux machines see: [https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-version-profiles-azurecli2?view=azs-2005\#linux-azure-ad](https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-version-profiles-azurecli2?view=azs-2005#linux-azure-ad)
...
## Use the Azure Kubernetes Service CLI to complete the following tasks:

### Create an Ubuntu Azure Kubernetes Service cluster:

Notice that the following steps are derived from the Azure documentation [here](https://docs.microsoft.com/en-us/azure/aks/kubernetes-walkthrough).
...
2.  Make sure you have a Service Principal identity ready with contributor permission on your subscription to create clusters in it.
    1.  To create an SPN using ADD follow these [instructions](https://docs.microsoft.com/en-us/azure-stack/operator/azure-stack-create-service-principals?view=azs-2005#create-a-service-principal-that-uses-a-client-secret-credential).
    2.  To create an SPN using ADFS follow these [instructions](https://docs.microsoft.com/en-us/azure-stack/operator/azure-stack-create-service-principals?view=azs-2005#create-a-service-principal-that-uses-client-secret-credentials).
    3.  To assaying "Contributor" role to the SPN see [instructions](https://docs.microsoft.com/en-us/azure-stack/operator/azure-stack-create-service-principals?view=azs-2005#assign-a-role). Make sure to select the "Contributor" role.
...
> [!NOTE]  
> The secret is highlighted in yellow in the text below
...
After the deploying a cluster and connect to it to verify it was deployed as expected, you can proceed to upgrade it. Notice that these instructions are derived from the following Azure [documentation](https://docs.microsoft.com/en-us/azure/aks/tutorial-kubernetes-upgrade-cluster)

1.  Get the available cluster versions by running:

    az aks get-upgrades --resource-group myResourceGroup --name myAKSCluster

    The output should contain the current version of each of the masters and agents and to which versions of Kubernetes they can be updated, it should look like this:
...
3.  s Service cluster.
    ...
    3\. Now you can proceed to repeat the tests above for Upgrade, Scale, deploy a Windows app, and Delete.
...
2.  Follow the instructions [here](https://docs.microsoft.com/en-us/azure/azure-arc/kubernetes/connect-cluster) to connect your cluster to Arc.
...
\<TODO: waiting for new env to test\>
...
2.  From a Linux machine, run the getkuberneteslogs.sh script available [here](https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-kubernetes-aks-engine-troubleshoot?view=azs-2005#collect-kubernetes-logs)
