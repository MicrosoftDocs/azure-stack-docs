...
|  | VPN Gateway SKUs | Support for Basic, Standard, High Performance, VPNGwy 1, VPNGwy 2, VPNGwy 3. | Ultra-High Performance is not supported. See [New virtual network gateways SKUs available](azure-stack-vpn-fast-path-user.md#new-virtual-network-gateways-skus-available-when-vpn-fast-path-is-enabled). |
