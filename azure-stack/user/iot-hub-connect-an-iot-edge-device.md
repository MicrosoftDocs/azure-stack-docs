...
Before you can complete this article, you must ensure the IoT Hub service is available in your subscription. If not, work with your administrator to [install the IoT Hub on Azure Stack Hub resource provider](../operator/iot-hub-rp-overview.md). The installation steps also cover the creation of an offer that includes the IoT Hub service. 
...
## Install and Configure IoT Edge device


### Deploy a new Linux VM on Azure Stack

1. Create a user account named `edgeadmin`.

### Install IoT Hub Root CA in trusted store on Edge device

1. Obtain a Root CA from IoTHub and store it in pem format.

2. Copy IoTHub hostname and paste it in a new tab in Google Chrome on Windows. Click on a lock icon near address bar, then `Certificates`. Select top most certificate (BBARoot) and then `View Certificate`. Save certificate in base-64 format with a name `BBARoot.crt`.
...
4. Verify the TLS connection is successfull from the Edge device using the following script:
   ...
   openssl s_client -connect $IOTHUB_HOSTNAME:443 -CAfile BBARoot.crt
   
   # install Root CA in the trusted store on Edge device
   sudo cp BBARoot.crt /usr/local/share/ca-certificates/
...
At this point, everything is ready to install and configure the IoT Edge runtime.

### Generate Device Certificates

#### Setup a tool to generate certificates
...
```
This script command creates several certificate and key files, but when articles ask for the root CA certificate, use the following file: `/home/edgeadmin/edged1/certs/azure-iot-test-only.root.ca.cert.pem`
...
#### Append IoTHub Root CA to the Device Root CA
```bash
# navigate to home directory
cd /home/edgeadmin

# append IoTHub Root CA to the Device Root CA
cat BBARoot.crt >> certs/azure-iot-test-only.root.ca.cert.pem
```
...
1. Navigate to IoT Hub on Azure Stack
...
# References
1. [Install the Azure IoT Edge runtime on Debian-based Linux systems](https://docs.microsoft.com/en-us/azure/iot-edge/how-to-install-iot-edge-linux)
1. [Configure an IoT Edge device to act as a transparent gateway](https://docs.microsoft.com/en-us/azure/iot-edge/how-to-create-transparent-gateway)
1. [Create demo certificates to test IoT Edge device features](https://docs.microsoft.com/en-us/azure/iot-edge/how-to-create-test-certificates)