...
You can install the Azure CLI to manage Azure Stack Hub with a Windows or Linux machines. This article walks you through the steps to install and set up Azure CLI.
...
1. Make a note of the CLI's Python location.

## Add certificate

Export and then import the Azure Stack Hub certificate for disconnected integrated systems. For connected integrated systems, the certificate is publicly signed and this step isn't necessary.
...
1. Register your Azure Stack Hub environment by running the `az cloud register` command.
   ...
   > [!NOTE]
   > Before you can sign in with `az login`, the Azure CLI must be configured to target your Azure Stack Hub instance rather than the default Azure public cloud. Use the `az cloud register` command to register your Azure Stack Hub environment and define its endpoints so the CLI knows where to send requests. After that, you can set the environment and then use `az login` to authenticate.
...
1. Register your environment. Use the following parameters when running `az cloud register`:
   ...
   | Resource Manager endpoint | `https://management.contoso.onmicrosoft.com` | The **ResourceManagerUrl** in integrated systems is: `https://management.<region>.<fqdn>/`. If you have a question about the integrated system endpoint, contact your cloud operator. |
   | Storage endpoint | local.contoso.onmicrosoft.com | For an integrated system, use an endpoint for your system.  |
   | Keyvault suffix | .vault.contoso.onmicrosoft.com | For an integrated system, use an endpoint for your system.  |
   ...
   For more information, see the [register command](/cli/azure/cloud#az-cloud-register) in the Azure CLI reference documentation.
...
1. Sign in to your Azure Stack Hub environment by using the `az login` command.
   ...
   You can either specify the username and password directly within the `az login` command, or authenticate by using a browser. You must authenticate using a browser if your account has multi-factor authentication enabled:
...
1. Verify that your environment is set correctly and that your environment is the active cloud:
...
This section walks you through setting up CLI if you use Active Directory Federated Services (AD FS) as your identity management service, and you use CLI on a Windows machine.
...
1. Register your environment. Use the following parameters when running `az cloud register`:
   ...
   | --- | --- | --- |
   | Environment name | AzureStackUser | Use `AzureStackUser`  for the user environment. If you're operator, specify `AzureStackAdmin`. |
   | Resource Manager endpoint | `https://management.local.azurestack.external` | The **ResourceManagerUrl** in integrated systems is: `https://management.<region>.<fqdn>/`. If you have a question about the integrated system endpoint, contact your cloud operator. |
   | Storage endpoint | local.azurestack.external | For an integrated system, use an endpoint for your system.  |
   | Keyvault suffix | .vault.local.azurestack.external | For an  integrated system, use an endpoint for your system.  |
   | VM image alias doc endpoint- | https://raw.githubusercontent.com/Azure/azure-rest-api-specs/master/arm-compute/quickstart-templates/aliases.json | URI of the document, which contains VM image aliases. |
...
1. Sign in to your Azure Stack Hub environment by using the `az login` command. You can sign in to the Azure Stack Hub environment either as a user or as a [service principal](../operator/give-app-access-to-resources.md).
...
1. Update your environment configuration to use the Azure Stack Hub specific API version profile. To update the configuration, run the following command:
   ...
   ```
    ...
    >[!NOTE]  
    >If you run a version of Azure Stack Hub earlier than the 1808 build, you must use the API version profile **2017-03-09-profile** rather than the API version profile **2020-09-01-hybrid**. You must also use a recent version of the Azure CLI.
...
1. Update your environment configuration to use the Azure Stack Hub specific API version profile. To update the configuration, run the following command:
   ...
   >[!NOTE]  
   >If you're running a version of Azure Stack Hub earlier than the 1808 build, you must use the API version profile **2017-03-09-profile** rather than the API version profile **2020-09-01-hybrid**. You also need to use a recent version of the Azure CLI.
...
- The CLI interactive mode isn't supported. For example, the `az interactive` command isn't yet supported in Azure Stack Hub.
...
These are the known Microsoft Graph issues for Azure CLI 2.40.0, or greater, for Azure Stack Hub. This primarily affects AD FS environments, as they don't support Microsoft Graph.

- `az keyvault create` interacts with Microsoft Graph. The following is an example workaround for AD FS. Primarily, the workaround uses the Azure AD Graph to retrieve user information such as the `objectId` rather than the Microsoft Graph.
  ...
  For more information about the Key Vault REST API, [see the Key Vault REST API reference](/rest/api/keyvault/).

### Other issues

The following are issues not limited to specific versions or ranges of versions of Azure CLI.

- `az role assignment create` isn't currently supported by Azure CLI for Azure Stack Hub due to an old API issue. The following workaround is required for both Microsoft Entra ID or AD FS.
  ...
  For more information about the role assignment REST API, [see the role assignments article](/rest/api/authorization/role-assignments).
