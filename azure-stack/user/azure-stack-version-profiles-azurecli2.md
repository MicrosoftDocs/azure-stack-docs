...
1. Sign in to your development workstation and install CLI. Azure Stack Hub requires version 2.0 or later of Azure CLI. 
  ...
  > [!IMPORTANT]
  > Due to a CVE affecting Azure CLI versions previous to 2.40.0 (https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-39327),
  It is no longer recommended that customers of Azure Stack Hub use Azure CLI 2.29.2 for ADFS. It is recommended to update to Azure CLI 2.40.0 or higher. However, ADFS customers will encounter issues with Azure CLI commands that interact with MS Graph endpoints. This is because MS Graph is not supported for ADFS. For workarounds to MS Graph issues, check the [General known issues](#general-known-issues) section.
...
These are the known Microsoft Graph issues for Azure CLI 2.40.0, or greater, for Azure Stack Hub. This primarily affects ADFS environments as it doesn't support MS Graph.

- `az keyvault create` interacts with MS Graph. The following is an example workaround for ADFS. Primarily, the workaround uses the Azure AD Graph to retrieve user information such as the `objectId` rather than the Microsoft Graph.
  ```powershell
