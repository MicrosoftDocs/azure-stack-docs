...
> [!NOTE]
> For `SPN k8s/apiserver@CONTOSO.com`, use the format, SPN k8s/apiserver@\<realm name>\. The \<realm name>\ is uppercase most of the time. However, if you are having issues, create the SPN with lowercase letters since Kerberos is case sensitive.
