...
In the configuration step, use [Set-AksHciConfig](/azure-stack/aks-hci/reference/ps/set-akshciconfig) to enable offline downloading with the `-offlineDownload` parameter. Then, specify the local path with the `-stagingShare` parameter. This is where the images will be downloaded.
