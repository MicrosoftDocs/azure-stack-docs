...
- You can now create disk snapshots without interrupting the IO workload in the running VM. Backup vendor solutions (Commvault and Veritas) will utilize the live snapshot functionality to provide backup of VMs with manage and unmanaged disks. We recommend use of backup solutions to ensure consistent backups of VMs and efficient use of live snapshot functionality. Work with you backup vendor to deploy the appropriate version of their solution that use of the live snapshot functionality. For managed disk snapshot, refer to [Managed Disk on Azure Stack Hub](../user/azure-stack-managed-disk-considerations.md). For unmanaged disk snapshot, refer to [Storage Consideration in Azure Stack Hub](../user/azure-stack-acs-differences.md).
