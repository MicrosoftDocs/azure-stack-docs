...
# Azure Event Hubs on Azure Stack Hub 1.2012.1.0 release notes
...
- The cluster health and backup Health Check for Event Hubs clusters weren't running
when clusters were in "Upgrading" or "Upgrade Failed" state. The issue is fixed in this release.
- Fixed a bug causing usage records to contain the wrong quantity. Instead of cores, the system was emitting capacity units (CU). Previously, a 1CU cluster showed 1 core in hourly usage. Users now see the correct quantity of 10 cores for a 1 CU cluster in their hourly usage.
