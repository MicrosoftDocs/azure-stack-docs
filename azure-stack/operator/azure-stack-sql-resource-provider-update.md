...
3.    The old **system.\<location\>.sqladapter** resource group and **system.\<location\>.sqladapter.dns** resource group in the default provider subscription will not be automatically deleted by the script. We recommend to keep the Storage Account in this resource group for some time. If after the upgrade, any tenant user observes inconsistent database or login metadata, it is possible to get support to restore the metadata from the Storage Account.
