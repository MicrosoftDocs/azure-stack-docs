...
For your Azure Stack Hub instance to remain in a supported state, the instance must run the most recently released update version or run either of the two preceding update versions (N-1, N-2). The following support restrictions apply to systems that aren't within our general two preceding update version support policy:

- Hotfixes for the platform are provided for the current version and and two preceding versions (N-1, N-2).
- Root Cause Analysis (RCAs) is provided for the current version and and two preceding versions (N-1, N-2).
