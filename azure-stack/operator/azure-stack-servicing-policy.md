...
- Issues on systems for unsupported versions (preceding N-2) are not entitled to receive support from Microsoft unless you're performing an update. Customer issues for supported versions are prioritized first. Support incidents for unsupported versions could mean longer than expected delays in response times.
