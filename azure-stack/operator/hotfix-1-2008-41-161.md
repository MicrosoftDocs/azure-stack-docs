...
description: Azure Stack Hub hotfix 1.2008.41.161 resolves secret rotation, ghost NIC, and disk space issues. Learn what's fixed and how to apply this update.
author: sethmanheim
...
- Removed overly verbose logging from Software Azure Load Balancer VMs.
...
- Fixed a bug in resource provider (for example, Azure Event Hubs) deployment, update, or secret rotation. The operation previously failed with no apparent failure cause. The fix allows the operation to complete successfully.
...
- Fixed an issue in which the copy of a certificate used by Azure Service Fabric was overwritten.
...
- Fixed an issue in internal secret rotation that caused a timeout error if value-add resource providers were unhealthy.
- Fixed a bug in which the operator portal updated the **Last Modified Time** for closed alerts even if the alert stayed closed.
...
- Fixed an issue in which Managed Disk usage data wasn't reported after the 2008 update.
- Fixed VMs losing connectivity while **SuspendNode** is occurring in Azure Stack, as part of host reboot during patch and update.
- Added PEP to retrieve current registration details and stale object cleanup for **Remove-Registration**.
...
- Fixed an issue in which a virtual subnet wasn't cleaned up if the tunnel was moved to a different GW VM and then the VGW was deleted.
- Fixed an issue that could cause registration and internal secret rotation to fail.
- Fixed an issue in internal secret rotation that might cause a failure in the next update.
...
- Remediated ERCS memory pressure during patch and update.
- Included **AzsInfraRoleSummary Test-Azurestack** test as **UpdateReadiness**.
- Fixed an issue where certificate rotation on Azure IoT Hub fails with "Internal execution error."
