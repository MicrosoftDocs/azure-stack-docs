...
> [!NOTE]
> When you exceed the **Maximum capacity (GB)** of another subscription, you can't create a new storage resource in this subscription. Although you can still use the unmanaged disks created in this subscription in VMs, doing so may cause your total used capacity to exceed the quota limit.<br>The maximum capacity of managed disks is separate from the total storage quota. You can set the total storage quota in **Compute quotas**.
