...
* We recommend to keep the Storage Account in the mysqladapter resource group for some time. If after the upgrade, any tenant user observes inconsistent database or login metadata, it is possible to get support to restore the metadata from the Storage Account.
* After verifying that the DNS Zone in the dbadapter.dns resource group is empty, it is safe to delete the dbadapter.dns resource group.
