...
description: Azure Stack Hub hotfix 1.2102.31.153 improves SDN stability, fixes VM and storage bugs, and enhances update reliability. Learn how to apply this update now.
author: sethmanheim
...
- CRP self-heals a VM with a SCSI disk that failed to attach instead of requiring operator removal of the disk from the VM.
...
- Resolved an issue in which the **Create a Virtual Machine** image dropdown displays image options that aren't available or weren't downloaded to the stamp.
...
- Resolved an issue with some Azure Key Vault applications being in an unhealthy state while updating from 2102 to later builds.
- Added retry for **get-volume** requests in **Test-AzureStack** infra capacity check.
- Shortened the "break-glass" tokens and made them human-readable.
...
- Fixed a bug that incorrectly raised a **PnPDevice.Attached** alert for a set of devices (mouse, keyboard, and other devices) that were safe.
...
- Extended the update readiness checks to cover more Azure Service Fabric health and VM health checks; for example, memory usage and storage disk capacity checks.
...
- Updated Microsoft Defender platform to version 4.18.2103.7.
- Enabled the alert module for customers depending on Syslog for alerts. The services continue to emit alerts to the Syslog pipeline.
