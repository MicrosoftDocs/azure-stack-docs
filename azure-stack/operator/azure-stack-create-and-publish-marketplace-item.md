...
1. Download the [Azure Gallery Packager tool](https://aka.ms/azsmarketplaceitem) and the sample Azure Stack gallery package. This download includes custom VM templates. Extract the .zip file and under the folder **Custom VMs** you can either use the Linux or Windows templates that are available. You can decide to re-use the pre-made templates and modify the respective parameters with the product details of the item that you will show on your Azure Stack portal or you can simply re-use the azpkg file available and skip the steps below to customize your own gallery package.
