...
- SQL Server 2016 SP1 or SP2 (Enterprise or Developer) on Windows Server 2016 server image. 
   > [!NOTE]
   > Standard version is not supported. When setting up the SQL Server AlwaysOn availabitlity group with SQL Server Standard version, only one DB can be created for one AG. [This limitation] (https://docs.microsoft.com/en-us/sql/database-engine/availability-groups/windows/basic-availability-groups-always-on-availability-groups?view=sql-server-ver15) makes Standard version unsuitable for this scenario. 
