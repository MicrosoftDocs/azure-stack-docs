...
description: Learn how to create Windows node pool in Azure Local.
ms.topic: how-to
author: rcheeran
ms.date: 09/22/2025
ms.author: rcheeran 
...
By default, a Kubernetes cluster is created with a node pool that can run Linux containers. Starting with Azure local 2509 release, Windows node pool feature is disabled by default. Users need to first enable the Windows node pool feature before you can create a Windows node pool. For more information, see the official documentation on [enabling Windows node pools](./howto-enable-windows-nodepools.md).
