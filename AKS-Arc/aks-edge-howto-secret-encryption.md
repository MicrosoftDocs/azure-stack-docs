...
This command can be used for AKS EE version 1.10.868.0 and later.
...
1. View the `readyz` API. If the problem persists, verify that the KMS provider is healthy. See the [Verify that the KMS provider is healthy](#verify-that-the-kms-provider-is-healthy) section.
