...
- **Maximum performance matters** — Workloads that cannot tolerate hypervisor overhead 
...
2. **Device Pool** — Groups machines for AKS workload placement (auto-creates a Custom Location)
3. **Logical Network** — Configures networking with IP pools for the cluster
4. **AKS cluster** — A fully managed Kubernetes cluster running directly on the hardware

All resources are managed through Azure Resource Manager and visible in the Azure portal.
